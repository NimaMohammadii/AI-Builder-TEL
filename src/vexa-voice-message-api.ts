...
export function registerVexaVoiceMessageRoutes(app: RouteApp): void {
  ...
  app.post('/app/api/vexa-voice/message', async (c) => {
    ...
    if (userId && await hasUserPlayedEvent(c.env, userId, playedKey)) {
      return c.json({ ok: false, reason: 'already_played' }, 200, { 'cache-control': VOICE_INDEX_CACHE });
    }
  ...
  app.post('/app/api/vexa-voice/played', async (c) => {
    ...
    await c.env.DB.prepare(`INSERT OR IGNORE INTO vexa_voice_user_events (user_id, event_id, played_at)
  ...
  app.get('/app/api/vexa-voice/audio/:event/:language', async (c) => {
    ...
    return new Response(object.body, {
      headers: {
        ...
        'cache-control': VOICE_AUDIO_CACHE,
        'accept-ranges': 'bytes',
      },
    });
  });
...
async function currentAdminMessage(env: EnvWithVexaVoice): Promise<AdminMessage | null> {
  ...
  try {
    ...
    if (!id) return null;
    return { id, title: cleanText(parsed.title, 120) || 'Vexa wants to say something', eventId: 'admin_message' };
  } catch {
    return null;
  }
}

async function readJson<T extends Record<string, unknown>>(c: RouteContext): Promise<T> {
  return c.req.json().catch(() => ({})) as Promise<T>;
}
...
function cleanEventKey(value: unknown): string {
  return String(value || '').replace(/[^0-9A-Za-z_-]/g, '').trim().slice(0, 90);
}
...
function isAdminRequest(c: RouteContext): boolean {
