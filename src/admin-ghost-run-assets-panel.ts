...
(function(){
  var assets=[
    ['background1','Moving background 1','First uploaded Ghost Run background layer.'],
    ['background2','Moving background 2','Second uploaded Ghost Run background layer.'],
    ['background3','Moving background 3','Third uploaded Ghost Run background layer.'],
  ...
  function addMenu(){
    ...
    section.innerHTML='<div class="row-title"><div><h2>Ghost Run Assets</h2><p class="muted small-text">Upload three moving backgrounds, ground, moon, idle ghost, moving ghost, three tree types and three house types. The idle and moving ghost images automatically switch during gameplay.</p></div></div><div class="ghost-assets-grid">'+assets.map(function(a){return '<div class="ghost-asset-card"><label>'+a[1]+'</label><img id="ghostAssetPreview_'+a[0]+'" src="'+api(a[0])+'" alt="'+a[1]+' preview"/><input id="ghostAssetInput_'+a[0]+'" type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml,.png,.jpg,.jpeg,.webp,.svg"/><button class="ghost" type="button" data-ghost-asset="'+a[0]+'">Upload '+a[1]+'</button><p class="muted small-text">'+a[2]+'</p></div>'}).join('')+'</div><p class="status" id="ghostRunAssetsStatus"></p>';
