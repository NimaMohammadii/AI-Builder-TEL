...
(function(){
  var assets=[
    ['background','Game background','Replaces the current Ghost Run scene background inside the game frame.'],
  ...
  function addMenu(){
    ...
    var btn=document.createElement('button');btn.className='menu-item';btn.type='button';btn.dataset.section='ghostRunAssets';btn.innerHTML='<strong>Ghost Run Assets</strong><span>Upload background, ground, moon, idle/moving ghost, trees and houses</span>';menu.appendChild(btn);
    ...
    section.innerHTML='<div class="row-title"><div><h2>Ghost Run Assets</h2><p class="muted small-text">Upload the game background, ground, moon, idle ghost, moving ghost, three tree types and three house types. The idle and moving ghost images automatically switch during gameplay.</p></div></div><div class="ghost-assets-grid">'+assets.map(function(a){return '<div class="ghost-asset-card"><label>'+a[1]+'</label><img id="ghostAssetPreview_'+a[0]+'" src="'+api(a[0])+'" alt="'+a[1]+' preview"/><input id="ghostAssetInput_'+a[0]+'" type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml,.png,.jpg,.jpeg,.webp,.svg"/><button class="ghost" type="button" data-ghost-asset="'+a[0]+'">Upload '+a[1]+'</button><p class="muted small-text">'+a[2]+'</p></div>'}).join('')+'</div><p class="status" id="ghostRunAssetsStatus"></p>';
