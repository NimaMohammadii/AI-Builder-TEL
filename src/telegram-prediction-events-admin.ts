  ...
  if (opsState) {
    ...
    try {
      ...
      if (opsState.mode === 'user-access') {
        const target = await resolvePredictOpsUser(env, text); await clearPredictOpsState(env, adminId); await sendPredictUserPanel(env, message.chat.id, undefined, target.telegram_user_id); return ok();
      }
...
async function handlePredictOpsCallback(env: Env, adminId: number, chatId: number, messageId: number | undefined, data: string): Promise<void> {
  ...
  if (data === 'botadmin:predictops:useraccess') { await savePredictOpsState(env, adminId, { mode: 'user-access' }); await upsert(env, chatId, messageId, '👤 User Predict Controls\n\nTelegram ID یا @username کاربر را بفرستید.', [[{ text: 'لغو', callback_data: 'botadmin:predictops:menu' }]]); return; }
  ...
  if (data.startsWith('botadmin:predictops:rrc:')) { const result = await manualRefundPredictRound(env, data.slice('botadmin:predictops:rrc:'.length), adminId); await publishPredictOpsRealtime(env, true); await sendPredictOpsRound(env, chatId, messageId, result.roundId, `✅ ${result.refundedCount} bet بازپرداخت شد • ${formatGram(result.refundedNano)} GRAM. برنده‌ها و refundهای قبلی دست‌نخورده ماندند.`); return; }
...
async function sendPredictOpsMenu(env: Env, chatId: number, messageId?: number, notice = ''): Promise<void> {
  const dashboard = await getPredictOpsDashboard(env), lines = dashboard.markets.flatMap((status) => predictOpsMarketSummaryLines(status, dashboard));
  const text = [notice, '🩺 Predict Operations', '', `Emergency: ${dashboard.emergencyPaused ? '🚨 PAUSED' : '✅ Normal'}`, `Maintenance: ${dashboard.maintenanceMessage || '—'}`, '', ...lines, '', 'Health → Access → Limits → Settlement. قیمت و منبع Aster از این پنل تغییر نمی‌کند.'].filter(Boolean).join('\n');
  const rows: Button[][] = [[{ text: '₿ Bitcoin', callback_data: 'botadmin:predictops:market:bitcoin' }, { text: '🥇 Gold', callback_data: 'botadmin:predictops:market:gold' }, { text: '🛢 Oil', callback_data: 'botadmin:predictops:market:oil' }], [{ text: dashboard.emergencyPaused ? '✅ Resume All Markets' : '🚨 Emergency Pause All', callback_data: `botadmin:predictops:emergency:${dashboard.emergencyPaused ? 'off' : 'on'}` }], [{ text: '👤 User Predict Controls', callback_data: 'botadmin:predictops:useraccess' }], [{ text: '🧾 Settlement Queue', callback_data: 'botadmin:predictops:queue' }, { text: '📜 Incident Log', callback_data: 'botadmin:predictops:incidents' }], [{ text: '🔐 Audit Log', callback_data: 'botadmin:predictops:audit' }, { text: '📝 Maintenance', callback_data: 'botadmin:predictops:askmaintenance' }], ...(dashboard.maintenanceMessage ? [[{ text: '🗑 حذف Maintenance Message', callback_data: 'botadmin:predictops:clearmaintenance' }]] : []), [{ text: '🔄 Refresh', callback_data: 'botadmin:predictops:refresh' }, { text: '⬅️ منوی اصلی', callback_data: 'botadmin:home' }]];
...
async function sendPredictOpsMarket(env: Env, chatId: number, messageId: number | undefined, market: PredictOpsMarket, notice = ''): Promise<void> {
  ...
  await upsert(env, chatId, messageId, text, [[{ text: status.manualPaused ? '✅ Resume Market' : '⏸ Pause Market', callback_data: `botadmin:predictops:marketpause:${market}:${status.manualPaused ? 'off' : 'on'}` }], [{ text: '⚖️ Exposure Limit', callback_data: `botadmin:predictops:ex:${market}` }], [{ text: '🔎 Round Inspector', callback_data: `botadmin:predictops:rounds:${market}` }, { text: '🧾 Settlement Queue', callback_data: 'botadmin:predictops:queue' }], [{ text: '⬅️ Predict Ops', callback_data: 'botadmin:predictops:menu' }]]);
}
...
async function sendPredictOpsRound(env: Env, chatId: number, messageId: number | undefined, roundId: string, notice = ''): Promise<void> {
  ...
  const rows: Button[][] = []; if (round.due) rows.push([{ text: '🔁 Retry Settlement', callback_data: `botadmin:predictops:retryask:${round.id}` }]); if (round.status === 'settled' && Number(round.counts.lost || 0) > 0) rows.push([{ text: '💸 Safety Refund Losing Bets', callback_data: `botadmin:predictops:rr:${round.id}` }]); rows.push([{ text: `⬅️ ${marketLabel(round.market)} Rounds`, callback_data: `botadmin:predictops:rounds:${round.market}` }]); await upsert(env, chatId, messageId, text, rows);
}

async function sendPredictRoundRefundConfirm(env: Env, chatId: number, messageId: number | undefined, roundId: string): Promise<void> { const round = await getPredictOpsRound(env, roundId); if (!round || round.status !== 'settled' || Number(round.counts.lost || 0) <= 0) throw new Error('این round برای Safety Refund واجد شرایط نیست.'); await upsert(env, chatId, messageId, `⚠️ Confirm Round Safety Refund\n\n${round.id}\nLost bets: ${Number(round.counts.lost || 0)}\n\nفقط اصل stake بت‌های Lost برگردانده می‌شود. Winnerها، payoutهای انجام‌شده و refundهای قبلی دست‌نخورده می‌مانند. هیچ debit یا clawback انجام نمی‌شود.`, [[{ text: '✅ Refund Losing Bets', callback_data: `botadmin:predictops:rrc:${round.id}` }], [{ text: 'لغو', callback_data: `botadmin:predictops:round:${round.id}` }]]); }
...
async function sendPredictOpsIncidents(env: Env, chatId: number, messageId?: number): Promise<void> { const incidents = await getPredictOpsIncidents(env); const lines = incidents.slice(0, 12).map((incident) => `${incidentIcon(incident.type)} ${formatOpsTime(incident.at)}${incident.market ? ` • ${marketLabel(incident.market)}` : ''}\n${shorten(incident.message, 150)}`); await upsert(env, chatId, messageId, `📜 Predict Incident Log\n\n${lines.length ? lines.join('\n\n') : 'هنوز incidentی ثبت نشده است.'}`, [[{ text: '🔄 Refresh', callback_data: 'botadmin:predictops:incidents' }, { text: '⬅️ Predict Ops', callback_data: 'botadmin:predictops:menu' }]]); }
...
async function resolvePredictOpsUser(env: Env, input: string): Promise<PredictOpsUserRow> { const value = String(input || '').trim(); if (/^\d{1,20}$/.test(value)) { const row = await findPredictOpsUserById(env, value); if (!row) throw new Error('کاربر پیدا نشد.'); return row; } const username = value.replace(/^@+/, '').trim(); if (!/^[0-9A-Za-z_]{3,64}$/.test(username)) throw new Error('ID یا username معتبر نیست.'); const row = await env.DB.prepare('SELECT telegram_user_id,username,first_name FROM app_users WHERE lower(username)=lower(?) ORDER BY datetime(COALESCE(last_seen_at,created_at)) DESC LIMIT 1').bind(username).first<PredictOpsUserRow>(); if (!row) throw new Error('کاربر پیدا نشد.'); return row; }
