...
(function(){
  ...
  function sentenceHtml(event){
    var name=esc(String(event&&event.displayName||'Player').trim()||'Player');
    var amount=esc(amountText(event));
    if(event.kind==='deposit')return '<span class="home-live-activity-muted">'+name+' </span><span class="home-live-activity-action action-deposit">deposited</span><span class="home-live-activity-muted"> '+amount+' GRAM</span>';
    if(event.kind==='withdraw')return '<span class="home-live-activity-muted">'+name+' requested a </span><span class="home-live-activity-action action-withdraw">withdrawal</span><span class="home-live-activity-muted"> of '+amount+' GRAM</span>';
    if(event.kind==='ticket'){
      var action=lowerFirst(event&&event.action);
      if(amount==='FREE')return '<span class="home-live-activity-muted">'+name+' </span><span class="home-live-activity-action action-ticket">claimed</span><span class="home-live-activity-muted"> a free ticket</span>';
      var match=action.match(/^bought\\s+(.+)$/i);
      var detail=esc(match&&match[1]?match[1]:'a ticket');
      return '<span class="home-live-activity-muted">'+name+' </span><span class="home-live-activity-action action-ticket">bought</span><span class="home-live-activity-muted"> '+detail+' for '+amount+' GRAM</span>';
    }
    return ''
  }
  function style(){
    ...
    var st=document.createElement('style');st.id='homeLiveActivityStyle';st.textContent=[
      ...
      '.home-live-activity-row{position:relative!important;overflow:hidden!important;height:32px!important;min-height:32px!important;border:0!important;outline:0!important;border-radius:28px!important;background:transparent!important;background-color:transparent!important;background-image:none!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.105),inset 0 -1px 0 rgba(255,255,255,.06),inset 0 0 22px rgba(255,255,255,.055),0 12px 28px rgba(0,0,0,.18)!important;display:flex!important;align-items:center!important;padding:0 10px!important;box-sizing:border-box!important;backdrop-filter:blur(3px) saturate(1.04)!important;-webkit-backdrop-filter:blur(3px) saturate(1.04)!important}.home-live-activity-row.is-new{animation:homeLiveEnter .46s cubic-bezier(.18,.82,.22,1)!important}.home-live-activity-row.is-shifting{animation:homeLiveShift .46s cubic-bezier(.18,.82,.22,1)!important}@keyframes homeLiveEnter{0%{opacity:0;transform:translateY(-12px) scale(.97)}100%{opacity:1;transform:none}}@keyframes homeLiveShift{0%{transform:translateY(-7px)}100%{transform:none}}',
      '.home-live-activity-copy{min-width:0!important;overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important;font-size:9px!important;line-height:1!important;font-weight:850!important;letter-spacing:-.01em!important}.home-live-activity-muted{color:rgba(255,255,255,.52)!important}.home-live-activity-action{font-weight:950!important}.home-live-activity-action.action-deposit{color:#66d68b!important}.home-live-activity-action.action-withdraw{color:#d7657a!important}.home-live-activity-action.action-ticket{color:#c76582!important}',
  ...
  function row(event,isNew,isShifting){return '<article class="home-live-activity-row kind-'+esc(event.kind)+(isNew?' is-new':'')+(isShifting?' is-shifting':'')+'" data-live-activity-id="'+esc(event.id)+'"><div class="home-live-activity-copy">'+sentenceHtml(event)+'</div></article>'}
