    ...
    const response = await fetch(`${OPENAI_BASE_URL}/responses`, {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          'Use edit_bot for any request that changes the connected Telegram bot behavior, menus, buttons, messages, forms, payments, or flow.',
          'A normal reply must never say a real bot change has been applied.',
          'Runtime source of truth is settings.flow only.',
          'If the user refers to their bot and a connected bot exists, choose the latest bot.',
