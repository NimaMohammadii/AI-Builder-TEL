...
const AUDIO_TYPES = new Set(['audio/mpeg', 'audio/mp3', 'audio/wav', 'audio/x-wav', 'audio/ogg', 'audio/webm', 'audio/mp4', 'audio/aac']);
...
app.get('/app/api/miniapp-audio-file', async (c) => getAssetResponse(c.env, MINIAPP_AUDIO_KEY, null));
...
async function uploadMiniappAudio(c: { env: Env; req: { formData: () => Promise<FormData>; header: (name: string) => string | undefined }; json: (data: Record<string, unknown>, status?: number) => Response }): Promise<Response> { if (!isAdminRequest(c)) return c.json({ error: 'Unauthorized. Login again.' }, 401); const form = await c.req.formData(); const file = form.get('audio'); if (!(file instanceof File)) return c.json({ error: 'Choose an audio file.' }, 400); if (!AUDIO_TYPES.has(file.type)) return c.json({ error: 'Only MP3, WAV, OGG, WEBM, MP4 or AAC audio files are allowed.' }, 400); if (file.size > 10_000_000) return c.json({ error: 'Audio must be under 10MB.' }, 400); const version = String(Date.now()); await c.env.ASSETS.put(MINIAPP_AUDIO_KEY, file.stream(), { httpMetadata: { contentType: file.type }, customMetadata: { version } }); await c.env.BOT_CACHE.put(MINIAPP_AUDIO_ENABLED_KEY, '0'); return c.json(await getMiniappAudioJson(c.env)); }
