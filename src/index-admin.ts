...
app.get('/admin/api/play-zone-cards', async (c) => { if (!isAdminRequest(c)) return c.json({ error: 'Unauthorized. Login again.' }, 401); return c.json(await getPlayZoneCardVisibility(c.env), 200, { 'cache-control': 'no-store' }); });
...
app.delete('/admin/api/section-lock-image', async (c) => { if (!isAdminRequest(c)) return c.json({ error: 'Unauthorized. Login again.' }, 401); try { const body = await c.req.json().catch(() => ({})); const section = normalizeSectionId(String(body.sectionId || '')); const kind = normalizeSectionImageKind(String(body.kind || 'locked')); await c.env.ASSETS.delete(sectionImageR2Key(section, kind)); await cleanupLegacyImageKv(c.env, [sectionImageKey(section, kind), sectionImageTypeKey(section, kind), sectionImageVersionKey(section, kind)]); return c.json(await getSectionLocks(c.env), 200, { 'cache-control': 'no-store' }); } catch (error) { return c.json({ error: error instanceof Error ? error.message : 'Could not delete image' }, 400, { 'cache-control': 'no-store' }); } });
