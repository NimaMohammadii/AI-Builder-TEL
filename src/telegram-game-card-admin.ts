...
import { getTelegramMenuMessageId } from './telegram-menu-state';
...
async function handleUpdate(env: Env, update: Update): Promise<Response | null> {
  ...
  try {
    ...
    if (target.kind === 'ton') {
      ...
      await tg<{ message_id?: number }>(token, 'sendPhoto', {
      ...
      await sendSavedImage(token, message.chat.id, `${PUBLIC_BASE_URL}/app/api/uploaded-image/slot-symbols/${target.symbol}?v=${Date.now()}`, `✅ ${slotSymbolLabel(target.symbol)} ذخیره شد و روی ریل‌های Slot نمایش داده می‌شود.`, '🎰 شکل‌های اسلات', 'botadmin:slotsymbols');
    } else {
      await tg<{ message_id?: number }>(token, 'sendPhoto', {
        chat_id: message.chat.id,
    ...
    // Keep the confirmation/preview in the history, but always put a fresh
    // admin home keyboard underneath it so the chat never ends at a bare
    // upload confirmation. The previously tracked prompt was deleted above;
    // sendAdminHome will replace that stale record with the new menu message.
    await sendHome(env, token, message.chat.id).catch(() => undefined);
  } catch (error) {
...
async function sendSavedImage(token: string, chatId: number, photo: string, caption: string, buttonText: string, callbackData: string): Promise<void> {
  ...
  await tg<{ message_id?: number }>(token, 'sendPhoto', { chat_id: chatId, photo, caption, reply_markup })
