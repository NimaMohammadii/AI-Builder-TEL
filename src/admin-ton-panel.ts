...
(function(){
  ...
  loadUsers=async function(){
    ...
    try{
      const r=await fetch('/admin/api/users',{credentials:'same-origin'}),j=await r.json().catch(()=>({error:'Invalid response'}));
      ...
      document.getElementById('statCredit').textContent=ton(j.stats?.totalTonBalanceNano??j.stats?.totalCredit??0);
  ...
  adjustCredit=async function(userId,deltaNano){
    ...
    try{
      const r=await fetch('/admin/api/users/ton-balance-adjust',{method:'POST',credentials:'same-origin',headers:{'content-type':'application/json'},body:JSON.stringify({userId,deltaNano})});
      const j=await r.json();
      if(!r.ok)throw new Error(j.error||'Could not update TON balance');
      const value=asNano(j.tonBalanceNano);
      const shown=ton(value);
      const user=adminUsers.find(u=>String(u.id)===String(userId));
      if(user){user.tonBalanceNano=value;user.tonBalance=shown}
      const row=document.getElementById('user-'+userId),node=row&&row.querySelector('.user-top>strong');
      if(node)node.textContent=shown;
      if(status)status.textContent='TON balance updated';
