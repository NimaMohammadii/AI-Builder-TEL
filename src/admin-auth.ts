...
export async function isAdminSession(env: Env, cookie: string | undefined): Promise<boolean> {
  ...
  return (await env.BOT_CACHE.get(`admin:session:${hash}`)) === '1';
}

export async function createAdminPasswordChallenge(env: Env): Promise<{ ok: true; challengeId: string; expiresIn: number } | { ok: false; error: string; status: number; retryAfter?: number }> {
  const lockKey = 'admin:2fa:lock';
  const lockedUntil = Number(await env.BOT_CACHE.get(lockKey) || 0);
  const now = Date.now();
  if (lockedUntil > now) return { ok: false, error: 'Too many wrong codes. Try again later.', status: 429, retryAfter: Math.ceil((lockedUntil - now) / 1000) };
  if (!env.BOT_ADMIN || !/^\d+$/.test(env.BOT_ADMIN)) return { ok: false, error: 'BOT_ADMIN numeric Telegram ID is not configured.', status: 500 };
  const token = telegramAdminBotToken(env);
  if (!token) return { ok: false, error: 'Telegram bot token is not configured.', status: 500 };

  const code = String(crypto.getRandomValues(new Uint32Array(1))[0] % 1_000_000).padStart(6, '0');
  const challengeId = randomToken(24);
  await env.BOT_CACHE.put(`admin:2fa:${challengeId}`, JSON.stringify({ codeHash: await sha256Hex(`${challengeId}:${code}:${env.ADMIN_KEY}`), attempts: 0, createdAt: now }), { expirationTtl: CODE_TTL_SECONDS });
  const sent = await sendTelegramCode(token, env.BOT_ADMIN, code);
  if (!sent.ok) {
...
export async function verifyAdminCode(env: Env, challengeId: string, code: string): Promise<{ ok: true; sessionToken: string } | { ok: false; error: string; status: number }> {
  ...
  if (cleanCode.length !== 6 || await sha256Hex(`${cleanId}:${cleanCode}:${env.ADMIN_KEY}`) !== challenge.codeHash) {
    ...
    await env.BOT_CACHE.put(key, JSON.stringify(challenge), { expirationTtl: Math.max(60, CODE_TTL_SECONDS - Math.floor((Date.now() - challenge.createdAt) / 1000)) });
...
function telegramAdminBotToken(env: Env): string {
  return env.GAME_BOT_TOKEN || env.AI_BOT_TOKEN || env.TELEGRAM_BOT_TOKEN || '';
}

async function sendTelegramCode(token: string, chatId: string, code: string): Promise<{ ok: boolean; description?: string }> {
  const response = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ chat_id: chatId, text: `Vexa admin login code: ${code}\nThis code expires in 5 minutes and can be used once.` }) });
