...
(function(){
  ...
  function ensurePanel(){
    ...
    section.hidden=true;
    section.innerHTML='<div class="row-title"><div><h2>Daily Rewards</h2><p class="muted small-text">Upload the large animated Rewards page image, upload one prize image for each of the 7 daily cards, then choose 6 missions for each day.</p></div><button id="dailyRewardsSave" class="primary" type="button">Save</button></div><div class="daily-rewards-admin-card"><h3>Rewards page top image</h3><p>This large image appears at the top of the mini app Rewards tab and gently floats up and down.</p><form class="daily-rewards-hero-form" data-daily-rewards-hero-image-form="1"><label>Top Rewards image</label><input id="dailyRewardsHeroImageFile" name="image" type="file" accept="image/png,image/jpeg,image/webp,image/svg+xml,.png,.jpg,.jpeg,.webp,.svg"/><button class="ghost" type="button" data-upload-daily-rewards-hero-image="1">Upload</button><button class="ghost danger" type="button" data-delete-daily-rewards-hero-image="1">Delete</button></form></div><div class="daily-rewards-admin-card"><h3>Daily Prize card images</h3><p>Each row controls the image shown on that day card in Home → Daily Prize.</p><div id="dailyRewardsDayImageForms" class="daily-rewards-admin-images"></div></div><div id="dailyRewardsDayTabs" class="daily-rewards-admin-tabs"></div><div id="dailyRewardsEditor" class="daily-rewards-admin-editor"></div><p id="dailyRewardsAdminStatus" class="status"></p>';
  ...
  async function uploadHeroImage(){
    ...
    var input=selectedHeroImage();
    var button=document.querySelector('[data-upload-daily-rewards-hero-image]');
    ...
    if(status)status.textContent='Uploading Rewards page top image...';
    if(button)button.disabled=true;
    try{
      var data=new FormData();
      data.append('image',file,file.name||'daily-rewards-hero');
      await postImageForm('/admin/api/upload-daily-rewards-hero-image',data);
      if(status)status.textContent='Rewards page top image uploaded.';
      input.value='';
