import { aiReply, defaultBlueprint, defaultFlow, improveFlow, type BotFlow, type ChatHistoryMessage } from './ai';
...
type ProgressPhase = 'accepted' | 'reading_dashboard' | 'reading_bot' | 'reading_flow' | 'planning_change' | 'generating_flow' | 'checking_flow' | 'saving_db' | 'clearing_cache' | 'publishing' | 'finalizing' | 'failed' | 'rejecting';
type ProgressFn = (phase: ProgressPhase, details?: unknown) => Promise<void>;
type ProgressTexts = Partial<Record<ProgressPhase, string>>;
...
async function executePending(env: Env, key: string, chatId: number, userId: string, pending: PendingAction, callback: TelegramCallbackQuery | null, confirmationText: string): Promise<void> {
  const historyKey = `builder-ai-history:${userId}`;
  const history = await loadHistory(env, historyKey);
  const progressTexts = await buildProgressTexts(env, pending, history);
  const progress: ProgressFn = async (phase, details) => updateProgress(key, chatId, callback, pending, phase, progressTexts, details);
...
async function rejectPending(env: Env, key: string, chatId: number, userId: string, pending: PendingAction | null, callback: TelegramCallbackQuery | null, userText: string): Promise<void> {
  const historyKey = `builder-ai-history:${userId}`;
  const history = await loadHistory(env, historyKey);
  const progressTexts = pending ? await buildProgressTexts(env, pending, history) : {};
  if (pending) await updateProgress(key, chatId, callback, pending, 'rejecting', progressTexts, { user_text: userText });
  await env.BOT_CACHE.delete(pendingKey(userId)).catch(() => undefined);
...
async function edit(env: Env, text: string, history: ChatHistoryMessage[], bots: BotView[], target: BotView | null, progress?: ProgressFn): Promise<ActionResult> {
  ...
  const generated = await improveFlow(env, flowNow, instruction);
  const flow = flowChangedLocal(flowNow, generated.flow) ? generated : fallbackSimpleFlow(flowNow, text);
  await progress?.('checking_flow', { flow_summary: flow.summary });
  const changed = flowChangedLocal(flowNow, flow.flow);
  ...
  try {
    ...
    await env.DB.prepare('UPDATE bots SET blueprint_json = ?, settings_json = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?').bind(JSON.stringify(bpNow), JSON.stringify(settings), full.id).run();
  ...
  return { ok: true, action: 'edit_bot', botId: full.id, flowChanged: true, flowSummary: flow.summary, blueprintSummary: 'Blueprint unchanged; runtime flow updated.' };
}
...
function flowChangedLocal(before: BotFlow, after: BotFlow): boolean { return JSON.stringify(before) !== JSON.stringify(after); }
function fallbackSimpleFlow(current: BotFlow, request: string): { flow: BotFlow; summary: string } { const buttonText = firstQuoted(request) || (/[\u0600-\u06FF]/.test(request) ? 'کانفیگ' : 'Config'); const nodeId = slug(buttonText); const message = secondQuoted(request) || (/[\u0600-\u06FF]/.test(request) ? 'باشه' : 'OK'); const flow = JSON.parse(JSON.stringify(current)) as BotFlow; const startId = flow.start || 'start'; const start = flow.nodes[startId] ?? { id: startId, message: 'Start', buttons: [] }; const buttons = Array.isArray(start.buttons) ? start.buttons.filter((b) => b.text !== buttonText) : []; buttons.push({ text: buttonText, next: nodeId }); flow.nodes[startId] = { ...start, id: startId, keyboard: start.keyboard ?? 'reply', buttons }; flow.nodes[nodeId] = { id: nodeId, message, end: true }; return { flow, summary: `Added ${buttonText} button and ${nodeId} node using deterministic fallback.` }; }
function firstQuoted(text: string): string | null { const m = text.match(/["“«']([^"”»']{2,40})["”»']/); return m?.[1]?.trim() || null; }
function secondQuoted(text: string): string | null { const matches = [...text.matchAll(/["“«']([^"”»']{2,80})["”»']/g)].map((m) => m[1].trim()); return matches[1] || null; }
function slug(text: string): string { const ascii = text.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, ''); if (ascii) return ascii.slice(0, 40); if (/کانفیگ|config/i.test(text)) return 'config'; return 'node_' + Math.random().toString(36).slice(2, 8); }
async function buildProgressTexts(env: Env, pending: PendingAction, history: ChatHistoryMessage[]): Promise<ProgressTexts> { const fallback = allFallbackProgress(pending); try { const response = await fetch(`${OPENAI_BASE_URL}/responses`, { method: 'POST', headers: { authorization: `Bearer ${env.OPENAI_API_KEY}`, 'content-type': 'application/json' }, body: JSON.stringify({ model: OPENAI_MODEL, instructions: 'Generate short live progress messages for a Telegram bot-builder operation. Return strict JSON object whose keys are phase names and values are natural short messages in the user language. Each message must start with exactly one relevant emoji. No markdown. Max 120 chars per value.', input: JSON.stringify({ phases: Object.keys(fallback), pending_action: pending, recent_history: history.slice(-4) }), max_output_tokens: 700, reasoning: { effort: 'low' } }) }); const data = (await response.json().catch(() => null)) as ResponsesApiResult | null; const text = data ? extractText(data) : null; if (!response.ok || !text) return fallback; const parsed = safeParseJson<ProgressTexts>(extractJson(text), {}); return { ...fallback, ...parsed }; } catch { return fallback; } }
function allFallbackProgress(pending: PendingAction): ProgressTexts { const phases: ProgressPhase[] = ['accepted', 'reading_dashboard', 'reading_bot', 'reading_flow', 'planning_change', 'generating_flow', 'checking_flow', 'saving_db', 'clearing_cache', 'publishing', 'finalizing', 'failed', 'rejecting']; return Object.fromEntries(phases.map((phase) => [phase, fallbackProgress(pending, phase)])) as ProgressTexts; }
function fallbackProgress(pending: PendingAction, phase: ProgressPhase): string { const fa = /[\u0600-\u06FF]/.test(`${pending.originalRequest} ${pending.proposalText}`); const t: Record<ProgressPhase, string> = { accepted: fa ? '✅ تأیید شد؛ شروع کردم.' : '✅ Confirmed. Starting now.', reading_dashboard: fa ? '🔎 دارم داشبورد و ربات‌های وصل‌شده رو بررسی می‌کنم.' : '🔎 Reading your dashboard and connected bots.', reading_bot: fa ? '🤖 دارم ربات هدف رو از دیتابیس می‌خونم.' : '🤖 Reading the target bot from the database.', reading_flow: fa ? '🧩 دارم flow، نودها و دکمه‌های واقعی رو بررسی می‌کنم.' : '🧩 Inspecting the real flow, nodes, and buttons.', planning_change: fa ? '🧠 دارم تغییر لازم رو با وضعیت فعلی ربات تطبیق می‌دم.' : '🧠 Matching the requested change with the current bot state.', generating_flow: fa ? '⚙️ دارم flow اجرایی جدید رو می‌سازم.' : '⚙️ Building the new executable flow.', checking_flow: fa ? '✅ دارم بررسی می‌کنم flow واقعاً تغییر کرده باشد.' : '✅ Checking that the flow actually changed.', saving_db: fa ? '💾 دارم تغییر واقعی رو در دیتابیس ذخیره می‌کنم.' : '💾 Saving the real change to the database.', clearing_cache: fa ? '♻️ دارم کش ربات رو پاک می‌کنم.' : '♻️ Clearing the bot cache.', publishing: fa ? '🌐 دارم webhook و وضعیت انتشار رو به‌روزرسانی می‌کنم.' : '🌐 Updating webhook and publish status.', finalizing: fa ? '🏁 دارم نتیجه نهایی رو آماده می‌کنم.' : '🏁 Preparing the final result.', failed: fa ? '⚠️ عملیات کامل نشد؛ دارم علت رو آماده می‌کنم.' : '⚠️ The operation did not finish; preparing the reason.', rejecting: fa ? '🛑 رد شد؛ دارم درخواست معلق رو حذف می‌کنم.' : '🛑 Rejected. Removing the pending request.' }; return t[phase]; }
async function updateProgress(key: string, chatId: number, callback: TelegramCallbackQuery | null, pending: PendingAction, phase: ProgressPhase, progressTexts: ProgressTexts, details: unknown): Promise<void> { const text = `${progressTexts[phase] ?? fallbackProgress(pending, phase)}`.slice(0, 300); await editOrSend(key, chatId, callback, pending, text); }
