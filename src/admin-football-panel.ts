...
(function(){
  ...
  function renderMatches(){
    ...
    list.innerHTML=matches.map(function(m){var qs=m.liveQuestions||[];return '<article class="football-admin-match-row"><b>'+esc(teamName(m.teamAId))+' vs '+esc(teamName(m.teamBId))+'</b><span>'+esc(m.startsAt)+' · '+esc(m.status||'')+(m.result?' · result '+esc(m.result):'')+'</span><div class="football-admin-match-actions"><button data-football-action="lock" data-id="'+esc(m.id)+'">Lock Now</button><button data-football-result="team_a" data-id="'+esc(m.id)+'">Team A Won</button><button data-football-result="draw" data-id="'+esc(m.id)+'">Draw</button><button data-football-result="team_b" data-id="'+esc(m.id)+'">Team B Won</button><button data-football-action="refund" data-id="'+esc(m.id)+'">Refund All</button></div><form class="football-admin-live-form" data-live-form="'+esc(m.id)+'"><input name="question" placeholder="Live question"/><input name="minutes" type="number" min="1" value="10"/><button type="submit">Add</button></form><div class="football-admin-live-list">'+(qs.length?qs.map(function(q){return '<div class="football-admin-live-row"><span>'+esc(q.question)+' · '+Math.ceil((q.remainingMs||0)/60000)+' min</span><div class="football-admin-match-actions"><button data-live-result="yes" data-qid="'+esc(q.id)+'">Yes wins</button><button data-live-result="no" data-qid="'+esc(q.id)+'">No wins</button><button data-live-action="refund" data-qid="'+esc(q.id)+'">Refund</button><button data-live-action="delete" data-qid="'+esc(q.id)+'">Delete</button></div></div>'}).join(''):'<span class="muted small-text">No active live questions.</span>')+'</div></article>'}).join('');
  ...
  async function deleteTeam(id){
    var status=document.getElementById('footballAdminStatus');if(!confirm('Delete this team?'))return;
