...
app.post('/admin/api/football-team-logo', async (c) => {
  ...
  try {
    ...
    await ensureFootballPredictTables(c.env);
    const team = await validateFootballTeam(c.env, String(form.get('team') || '')); 
...
app.post('/admin/api/football-matches', async (c) => {
  ...
  try {
    await ensureFootballPredictTables(c.env);
    const body = await c.req.json().catch(() => ({})) as Record<string, unknown>;
    const teamA = await validateFootballTeam(c.env, String(body.teamAId || body.team_a_id || ''));
    const teamB = await validateFootballTeam(c.env, String(body.teamBId || body.team_b_id || ''));
...
app.post('/admin/api/football-matches/:id/live-questions', async (c) => {
  ...
  try {
    await ensureFootballPredictTables(c.env);
    await expireFootballLiveQuestions(c.env);
    const matchId = cleanDbText(c.req.param('id'), 'Missing match id');
    const match = await c.env.DB.prepare("SELECT * FROM football_matches WHERE id = ? AND status != 'cancelled'").bind(matchId).first<FootballMatchRow>();
    if (!match) throw new Error('Match not found');
    const body = await c.req.json().catch(() => ({})) as Record<string, unknown>;
    const text = cleanQuestionText(body.question || body.questionText || body.text);
    const minutes = normalizeTimerMinutes(body.timerMinutes || body.minutes || body.durationMinutes);
    const id = 'flq_' + crypto.randomUUID().replace(/-/g, '').slice(0, 24);
...
app.post('/admin/api/football-live-questions/:id/action', async (c) => {
  ...
  try {
    ...
    const body = await c.req.json().catch(() => ({})) as Record<string, unknown>;
    const action = String(body.action || '').trim().toLowerCase();
    const question = await c.env.DB.prepare('SELECT * FROM football_live_questions WHERE id = ?').bind(id).first<FootballLiveQuestionRow>();
    if (!question) throw new Error('Live question not found');
    if (action === 'resolve') await settleFootballLiveQuestion(c.env, id, normalizeLivePick(body.result));
    else if (action === 'refund') await refundFootballLiveQuestion(c.env, id);
