...
export type TonTransactionKind = 'deposit' | 'withdraw' | 'game' | 'group_usage' | 'admin' | 'adjustment' | 'market' | 'market_refund' | 'predict' | 'world_cup_predict' | 'tts' | 'tts_refund' | 'referral';
...
function titleForKind(kind: string, amountNano: number): string {
  ...
  if (kind === 'world_cup_predict') return amountNano >= 0 ? 'World Cup prediction payout' : 'World Cup prediction stake';
...
function cleanKind(value: unknown): TonTransactionKind {
  ...
  return ['deposit', 'withdraw', 'game', 'group_usage', 'admin', 'adjustment', 'market', 'market_refund', 'predict', 'world_cup_predict', 'tts', 'tts_refund', 'referral'].includes(raw) ? raw : 'adjustment';
}
