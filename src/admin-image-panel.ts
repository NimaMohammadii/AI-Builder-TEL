...
(function(){
  ...
  function render(){
    ...
    document.querySelectorAll('[data-lock-img-delete]').forEach(btn=>btn.onclick=()=>deleteImage(btn.getAttribute('data-lock-img-delete'),btn.getAttribute('data-kind')));
  }
  async function load(){
  ...
  async function deleteImage(sectionId,kind){
    const status=document.getElementById('adminLockImageStatus');
    if(status)status.textContent='Deleting image...';
    try{const r=await fetch('/admin/api/section-lock-image',{method:'DELETE',credentials:'same-origin',headers:{'content-type':'application/json'},body:JSON.stringify({sectionId:sectionId||SHARED_SECTION_ID,kind:kind||'locked'})});const j=await r.json();if(!r.ok)throw new Error(j.error||'Could not delete image');findSharedImages(j.sections||[]);render();if(status)status.textContent='Image deleted.';}catch(e){if(status)status.textContent=e.message||'Could not delete image';}
  }
