...
#ghostrun .crash-live{margin-top:28px!important}
...
(function(){
  ...
  function currentTelegramUser(){try{var u=window.Telegram&&window.Telegram.WebApp&&window.Telegram.WebApp.initDataUnsafe&&window.Telegram.WebApp.initDataUnsafe.user;if(!u||!u.id)return null;return {name:u.username?'@'+u.username:(u.first_name||'You'),bets:[{amount:Math.max(.1,nanoToTon(placedBetNano||activeBetNano||tonToNano(betInput&&betInput.value||.1))),cashoutMultiplier:1.22+Math.random()*.55}],real:true}}catch(e){return null}}
  ...
  function prepareLive(){var fallback=fallbackUsers();function build(users){users=Array.isArray(users)&&users.length?users:fallback;var me=currentTelegramUser();if(me&&!users.some(function(u){return String(u&&u.name||'')===me.name}))users=[me].concat(users);liveRows=users.slice(0,24).map(function(u,i){var b=pickBet(u,i);var second=Math.max(0,Math.min(8,Number(u.betSecond)||0));return {id:liveSeq++,name:u.name||'Player',amount:Number(b.amount)||.1,target:Number(b.cashoutMultiplier)||1.4,betSecond:second,visible:second<=0,status:'bet'}});revealLive(false);renderLive()}if(liveLoaded&&window.__ghostLiveUsers){build(window.__ghostLiveUsers);return}fetch('/app/api/ghost-run-virtual-users',{cache:'no-store'}).then(function(r){return r.ok?r.json():null}).then(function(j){window.__ghostLiveUsers=j&&Array.isArray(j.users)?j.users:fallback;liveLoaded=true;build(window.__ghostLiveUsers)}).catch(function(){build(fallback)})}
