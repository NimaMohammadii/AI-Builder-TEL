...
(function(){
  ...
  function currentTelegramUser(){try{var u=window.Telegram&&window.Telegram.WebApp&&window.Telegram.WebApp.initDataUnsafe&&window.Telegram.WebApp.initDataUnsafe.user;if(!u||!u.id)return null;return {id:String(u.id),name:u.username?'@'+u.username:(u.first_name||'You'),bets:[{amount:Math.max(.1,nanoToTon(placedBetNano||activeBetNano||tonToNano(betInput&&betInput.value||.1))),cashoutMultiplier:1.22+Math.random()*.55}],real:true}}catch(e){return null}}
