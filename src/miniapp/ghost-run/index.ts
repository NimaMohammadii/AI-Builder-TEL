...
(function(){
  ...
  function prepareLive(){var fallback=fallbackUsers();function build(users){users=Array.isArray(users)&&users.length?users:fallback;var me=currentTelegramUser();if(me&&!users.some(function(u){return String(u&&u.name||'')===me.name}))users=[me].concat(users);liveRows=users.slice(0,24).map(function(u,i){var b=pickBet(u,i);return {id:liveSeq++,name:u.name||'Player',amount:Number(b.amount)||.1,target:Number(b.cashoutMultiplier)||1.4,status:'bet'}});renderLive()}if(liveLoaded&&window.__ghostLiveUsers){build(window.__ghostLiveUsers);return}fetch('/app/api/ghost-run-virtual-users',{cache:'no-store'}).then(function(r){return r.ok?r.json():null}).then(function(j){window.__ghostLiveUsers=j&&Array.isArray(j.users)?j.users:fallback;liveLoaded=true;build(window.__ghostLiveUsers)}).catch(function(){build(fallback)})}
  function updateLive(){var changed=false;liveRows.forEach(function(r){if(r.status==='bet'&&Number(r.target)<=multiplier){r.status='cashout';r.cashoutMultiplier=Number(r.target);changed=true}});if(changed)renderLive()}
  function crashLive(){var changed=false;liveRows.forEach(function(r){if(r.status==='bet'){r.status='crashed';changed=true}});if(changed)renderLive()}
  ...
  function tick(now){if(phase==='betting')render();if(phase==='running'){var t=(now-phaseStartedAt)/1000;multiplier=1+t*.038+Math.pow(t,1.12)*.020;var center=Math.max(18,viewportWidth()*0.5-42);var walkSpeed=25;position=Math.min(center,18+t*walkSpeed);backgroundOffset=Math.max(0,t-((center-18)/walkSpeed))*17;updateLive();if(multiplier>=crashPoint)endRound();else render()}raf=requestAnimationFrame(tick)}
