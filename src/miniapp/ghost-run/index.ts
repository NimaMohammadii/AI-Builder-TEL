...
(function(){
  ...
  function tick(now){if(phase==='betting'){revealLive(false);render();}if(phase==='running'){var t=(now-phaseStartedAt)/1000;multiplier=1+t*.038+Math.pow(t,1.12)*.020;var center=Math.max(18,viewportWidth()*0.5-42);var walkSpeed=25;position=Math.min(center,18+t*walkSpeed);backgroundOffset=Math.max(0,t-((center-18)/walkSpeed))*17;updateLive();if(multiplier>=crashPoint)endRound();else render()}raf=requestAnimationFrame(tick)}
