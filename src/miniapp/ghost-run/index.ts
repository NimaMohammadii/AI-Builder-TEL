  ...
  const safeAssetLoader = `function cssUrl(url){var clean=String(url||'').split("'").join('').split(')') .join('').split('"').join('');return "url('"+clean+"')"}
    ...
    function injectAssetUrls(urls){
      ...
      style.textContent=[
        ...
        "#ghostrun .ghost-run-screen[data-ghost-state='idle'] .ghost-run-ghost{background-image:"+cssUrl(urls.ghostidle)+"!important;animation:ghostRunIdleBreath 1.8s ease-in-out infinite!important;filter:drop-shadow(0 0 10px rgba(255,255,255,.10))!important}",
        "#ghostrun .ghost-run-screen[data-ghost-state='moving'] .ghost-run-ghost{background-image:"+cssUrl(urls.ghostmove)+"!important;animation:ghostRunMoveBob .38s ease-in-out infinite alternate!important;filter:drop-shadow(0 0 16px rgba(255,255,255,.18))!important}",
        ...
        "@keyframes ghostRunIdleBreath{0%,100%{transform:translate3d(0,0,0) scale(1);filter:drop-shadow(0 0 9px rgba(255,255,255,.10))}50%{transform:translate3d(0,-3px,0) scale(1.01);filter:drop-shadow(0 0 15px rgba(255,255,255,.16))}}",
