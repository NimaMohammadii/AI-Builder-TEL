...
<section id="ghostrun" class="view ghost-run-view" aria-label="Ghost Run">
  ...
  (function(){
    ...
    function updateMultiplier(dt){if(direction>0){multiplierValue+=dt*(0.055+stageIndex()*0.014)+(Math.max(0,distance)*0.0000015)}else if(direction<0){multiplierValue=Math.max(1,multiplierValue-dt*(0.024+stageIndex()*0.004))}}
