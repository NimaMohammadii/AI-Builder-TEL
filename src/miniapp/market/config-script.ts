...
(function(){
  ...
  function openMarket(){initMarketTabs();setMarketTab('store')}
  document.addEventListener('keydown',function(e){if(e.key==='Escape')closeDetail()});
  document.addEventListener('click',function(e){var b=e.target&&e.target.closest&&e.target.closest('[data-view="market"]');if(b)setTimeout(openMarket,120)},true);
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){initMarketTabs()});else initMarketTabs();
})();
`;