...
#rewards .rewards-live-winners{
  margin-top:clamp(250px,42dvh,460px)!important;
...
#rewards .home-live-winner-card{
  ...
  transform:translate3d(0,calc(var(--rewards-card-progress) * -34px),0) scale(calc(1 - var(--rewards-card-progress) * .08))!important;
  filter:blur(calc(var(--rewards-card-progress) * 8px))!important;
  transform-origin:50% 0!important;
  will-change:transform,opacity,filter!important;
  transition:opacity .16s linear,transform .16s linear,filter .16s linear!important;
  pointer-events:auto;
}
#rewards .home-live-winner-card[data-rewards-hidden="1"]{pointer-events:none!important}
@media(max-width:380px){#rewards .rewards-live-winners{margin-top:clamp(225px,40dvh,410px)!important}}
