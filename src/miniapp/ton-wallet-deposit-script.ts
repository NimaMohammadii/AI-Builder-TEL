...
(function(){
  ...
  function cleanAmount(value){var raw=normalizeNumericText(value);if(!raw||raw==='.')return 0;var n=Number(raw);return Number.isFinite(n)&&n>0?Math.floor(n*NANO_PER_TON)/NANO_PER_TON:0}
  ...
  function installInvoiceDoneHook(){if(invoiceHooked||!tg||typeof tg.openInvoice!=='function')return;invoiceHooked=true;var original=tg.openInvoice.bind(tg);tg.openInvoice=function(link,callback){return original(link,function(state){try{if(typeof callback==='function')callback(state)}catch(e){}if(state==='paid')showPaymentDone()})}}
  ...
  function ensureTonInput(){
    var stars=starsInput();if(!stars)return null;
    var ton=tonInput();
    if(!ton){ton=document.createElement('input');ton.id='tonAmountSheet';ton.type='text';ton.inputMode='decimal';ton.setAttribute('inputmode','decimal');ton.setAttribute('autocomplete','off');ton.setAttribute('autocorrect','off');ton.placeholder='TON amount';ton.value='0.1';ton.style.display='none';stars.insertAdjacentElement('afterend',ton)}
    return ton;
  }
  function currentTonText(){var input=ensureTonInput();return normalizeNumericText(input&&input.value)}
  function starsToTon(stars){return stars>0?stars*STARS_TO_NANO/NANO_PER_TON:0}
  function setMode(mode){
    depositMode=mode==='ton'?'ton':'stars';
    var stars=starsInput();var ton=ensureTonInput();var label=document.querySelector('#depositSheet .deposit-custom-field label');
    ...
    if(stars){stars.style.display=depositMode==='ton'?'none':'';stars.inputMode='numeric';stars.setAttribute('inputmode','numeric')}
    if(ton){ton.style.display=depositMode==='ton'?'':'none'}
    ...
    syncModeUi();if(depositMode==='ton'&&!tonUsd)loadTonUsd();
  }
  function syncModeUi(){
    var out=q('starsTonEquivalent');if(!out)return;
    if(depositMode==='ton'){
      var amount=cleanAmount(currentTonText());var usd=tonUsd&&amount?amount*tonUsd:0;out.textContent=usd?'≈ $'+usd.toFixed(2):'USD';out.classList.add('usd-mode');
    }else{
      var s=starsInput();var stars=Math.max(0,Math.floor(Number(normalizeNumericText(s&&s.value))||0));var ton=starsToTon(stars);out.textContent=stars>0?'≈ '+shortTon(ton)+' TON':'≈ 0 TON';out.classList.remove('usd-mode');
    }
  }
  ...
  function bind(){
    ensureUi();
    document.addEventListener('input',function(ev){if(ev.target&&['starsAmountSheet','tonAmountSheet'].indexOf(ev.target.id)!==-1)syncModeUi()});
    document.addEventListener('focusin',function(ev){if(ev.target&&['starsAmountSheet','tonAmountSheet'].indexOf(ev.target.id)!==-1){lockDepositHeight();setTimeout(function(){window.scrollTo(0,0)},40);setTimeout(function(){window.scrollTo(0,0)},180)}});
    document.addEventListener('click',function(ev){var button=ev.target&&ev.target.closest?ev.target.closest('button'):null;if(!button)return;var action=button.getAttribute('data-action');if(action==='open-deposit'){lockDepositHeight();setTimeout(ensureUi,40)}if(action==='set-deposit-mode'){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation&&ev.stopImmediatePropagation();setMode(button.getAttribute('data-mode'))}if(action==='confirm-ton-payment'){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation&&ev.stopImmediatePropagation();confirmTonPayment()}},true);
    setTimeout(ensureUi,220);setTimeout(ensureUi,900);
  }
