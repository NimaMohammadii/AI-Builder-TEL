...
(function(){
  ...
  function showPendingControls(show){var verify=q('tonWalletVerifyButton');if(verify){verify.hidden=!show;verify.style.display=show?'block':'none'}}
  ...
  function openPayUrl(url){if(!url)return;try{window.location.href=url;return}catch(e){}try{if(tg&&typeof tg.openLink==='function')tg.openLink(url)}catch(e){}}
  ...
  async function verifyDeposit(id,manual){
    ...
    if(!depositId){showPendingControls(false);if(manual)toast('No pending TON payment');return}
    if(verifying)return;
    verifying=true;
  ...
  function schedulePendingVerify(){if(verifyTimer)return;verifyTimer=setInterval(function(){var p=readPending();if(p&&p.id)verifyDeposit(p.id,false);else{clearInterval(verifyTimer);verifyTimer=0;showPendingControls(false)}},VERIFY_INTERVAL_MS)}
