...
(function(){
  ...
  function cycleFor(id){var stop=roundStop(id);var runMs=Math.max(620,Math.min(MAX_RUN_MS,stopTime(stop)));return{id:id,stop:stop,runMs:runMs,cycleMs:runMs+WAIT_BETWEEN_MS}}
  function locateRound(now){var elapsed=Math.max(0,now-SCHEDULE_EPOCH);var approx=Math.max(0,Math.floor(elapsed/(WAIT_BETWEEN_MS+3600))-40);var t=SCHEDULE_EPOCH;for(var i=0;i<approx;i++){t+=cycleFor(i).cycleMs}var id=approx;while(true){var c=cycleFor(id);if(now<t+c.cycleMs){var local=now-t;var running=local<c.runMs;var waitElapsed=running?0:local-c.runMs;var nextIn=running?0:Math.max(0,WAIT_BETWEEN_MS-waitElapsed);return{id:id,start:t,local:local,runElapsed:Math.min(local,c.runMs),waitElapsed:waitElapsed,stop:c.stop,runMs:c.runMs,running:running,waiting:!running,nextIn:nextIn}}t+=c.cycleMs;id++}}
  ...
  function progressAt(runElapsed){return Math.min(.995,runElapsed/8400)}
  ...
  function drawGraph(progress,ended){var now=performance.now();if(!ended&&now-lastDraw<18)return;lastDraw=now;var ctx=getCanvas();if(!ctx)return;var w=ctx.__w,h=ctx.__h;ctx.clearRect(0,0,w,h);drawGrid(ctx,w,h);var visual=ended?chartCurrent:current;var left=4,bottom=h-8,right=w-16,top=8;var maxM=Math.max(3.25,visual,4.8);var p=Math.min(1,Math.max(0,progress));lastProgress=p;var easeP=1-Math.pow(1-Math.min(.995,p),1.08);var tipX=left+(right-left)*easeP;var normalized=Math.min(1,(Math.max(1,visual)-1)/(Math.max(3.25,maxM)-1));var visualY=Math.pow(Math.max(.02,normalized),1.42);var tipY=bottom-(bottom-top)*Math.max(.022,visualY);var points=[];for(var i=0;i<=110;i++){var tt=i/110;var px=left+(tipX-left)*tt;var py=bottom-(bottom-tipY)*Math.pow(tt,1.95);points.push([px,py])}ctx.save();ctx.beginPath();ctx.moveTo(left,bottom);for(var a=0;a<points.length;a++)ctx.lineTo(points[a][0],points[a][1]);ctx.lineTo(tipX,h);ctx.lineTo(left,h);ctx.closePath();var shadow=ctx.createLinearGradient(0,top,0,h);shadow.addColorStop(0,ended?'rgba(255,125,145,.30)':'rgba(255,255,255,.31)');shadow.addColorStop(.42,ended?'rgba(255,125,145,.14)':'rgba(255,255,255,.145)');shadow.addColorStop(1,'rgba(255,255,255,0)');ctx.fillStyle=shadow;ctx.globalAlpha=.86;ctx.fill();ctx.restore();ctx.save();var line=ctx.createLinearGradient(left,bottom,tipX,tipY);line.addColorStop(0,'rgba(255,255,255,.32)');line.addColorStop(.55,'rgba(255,255,255,1)');line.addColorStop(1,ended?'rgba(255,125,145,.72)':'rgba(255,255,255,.64)');ctx.strokeStyle=line;ctx.lineWidth=4.2;ctx.lineCap='round';ctx.lineJoin='round';ctx.shadowColor='transparent';ctx.shadowBlur=0;ctx.beginPath();ctx.moveTo(left,bottom);for(var c=0;c<points.length;c++)ctx.lineTo(points[c][0],points[c][1]);ctx.stroke();ctx.restore();drawTip(ctx,tipX,tipY,ended);if(!ended)chartCurrent=visual}
