...
(function(){
  ...
  function loadCrashTipImage(){
    ...
    var img=new Image();
    img.decoding='async';
    img.onload=function(){crashTipReady=true;draw(running?Math.min(.985,current/Math.max(2.5,stopAt)):0,true)};
    img.onerror=function(){crashTipFailed=true;crashTipReady=false};
    img.src='/app/api/crash-tip-image.png?v='+Date.now();
    crashTipImage=img;
  }
  ...
  function drawTip(ctx,x,y,ended){
    if(crashTipReady&&crashTipImage&&crashTipImage.naturalWidth){
      ...
      ctx.shadowBlur=14;
      ctx.drawImage(crashTipImage,x-size/2,y-size/2,size,size);
      ctx.restore();
      return;
    }
    ctx.fillStyle=ended?'rgba(255,255,255,.55)':'#fff';
    ctx.beginPath();
    ctx.arc(x,y,5.5,0,Math.PI*2);
    ctx.fill();
  }
  function draw(progress, ended){var now=performance.now();if(!ended&&now-lastDraw<32)return;lastDraw=now;var ctx=getCanvas();if(!ctx)return;var w=320,h=290;ctx.clearRect(0,0,w,h);ctx.strokeStyle='rgba(255,255,255,.055)';ctx.lineWidth=1;for(var x=24;x<w;x+=42){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,h);ctx.stroke()}for(var y=36;y<h;y+=42){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(w,y);ctx.stroke()}var left=24,bottom=244,right=296,top=52;var p=Math.min(1,Math.max(0,progress));var maxX=left+(right-left)*p;var tipY=bottom-(bottom-top)*Math.min(1,p*.92+.08);ctx.strokeStyle=ended?'rgba(255,255,255,.38)':'rgba(255,255,255,.92)';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(left,bottom);for(var i=0;i<=38;i++){var t=i/38;var px=left+(maxX-left)*t;var py=bottom-(bottom-top)*Math.pow(t,1.65)*Math.min(1,p*.92+.08);ctx.lineTo(px,py)}ctx.stroke();drawTip(ctx,maxX,tipY,ended)}
  ...
  function bind(){loadCrashTipImage();mult(1);status('Ready');draw(0,true);buttons();var s=q('crashStart'),c=q('crashCashout'),input=q('crashAmount');if(s)s.onclick=start;if(c)c.onclick=cashout;if(input)input.addEventListener('change',normalizeAmount);document.addEventListener('click',function(ev){var b=ev.target&&ev.target.closest&&ev.target.closest('button');if(!b)return;var a=b.getAttribute('data-action');if(a==='crash-half'){ev.preventDefault();half()}if(a==='crash-double'){ev.preventDefault();doubleAmount()}});window.addEventListener('resize',function(){draw(running?Math.min(.985,current/Math.max(2.5,stopAt)):0,true)})}
