...
(function(){
  ...
  function drawGraph(progress,ended){
    ...
    var normalized=Math.min(1,(Math.max(1,current)-1)/(Math.max(3.2,maxM)-1));
    ...
    var fill=ctx.createLinearGradient(0,top,0,h);fill.addColorStop(0,ended?'rgba(255,125,145,.10)':'rgba(255,255,255,.10)');fill.addColorStop(1,'rgba(255,255,255,0)');ctx.fillStyle=fill;ctx.globalAlpha=.72;ctx.fill();
    ctx.restore();
  ...
  function finish(){running=false;phase='crashed';history.unshift(stopAt);history=history.slice(0,12);renderHistory();status(cashed?'Round ended':'Crashed');setCountdown('Crashed',false);buttons();drawGraph(lastProgress,true);setTimeout(function(){if(phase==='crashed'){phase='ready';setCountdown('Ready',false);buttons();drawGraph(0,true)}},1800)}
  function tick(now){if(phase!=='running')return;var elapsed=(now-startTime)/1000;setTotal(elapsed);current=1+elapsed*.22+elapsed*elapsed*.055;if(current>=stopAt){current=stopAt;mult(current);finish();return}mult(current);drawGraph(Math.min(.985,elapsed/7.2),false);requestAnimationFrame(tick)}
