...
(function(){
  ...
  function roundState(now){var id=Math.floor(now/ROUND_MS);var start=id*ROUND_MS;var elapsed=now-start;var stop=roundStop(id);var crashAt=Math.max(620,Math.min(ROUND_MS-CRASH_HOLD_MS,stopTime(stop)));var crashed=elapsed>=crashAt;var nextIn=Math.max(0,ROUND_MS-elapsed);return{id:id,start:start,elapsed:elapsed,stop:stop,crashAt:crashAt,crashed:crashed,nextIn:nextIn}}
  function stopTime(stop){var target=Math.max(1,Number(stop)||1);var lo=0,hi=ROUND_MS-CRASH_HOLD_MS;for(var i=0;i<24;i++){var mid=(lo+hi)/2;if(multAt(mid/1000)>=target)hi=mid;else lo=mid}return hi}
  function multAt(seconds){return 1+seconds*.155+seconds*seconds*.036}
  function progressAt(elapsed){return Math.min(.995,elapsed/8400)}
  ...
  function drawGraph(progress,ended){var now=performance.now();if(!ended&&now-lastDraw<18)return;lastDraw=now;var ctx=getCanvas();if(!ctx)return;var w=ctx.__w,h=ctx.__h;ctx.clearRect(0,0,w,h);drawGrid(ctx,w,h);var visual=ended?chartCurrent:current;var left=4,bottom=h-8,right=w-16,top=8;var maxM=Math.max(3.25,visual,4.8);var p=Math.min(1,Math.max(0,progress));lastProgress=p;var easeP=1-Math.pow(1-Math.min(.995,p),1.08);var tipX=left+(right-left)*easeP;var normalized=Math.min(1,(Math.max(1,visual)-1)/(Math.max(3.25,maxM)-1));var visualY=Math.pow(Math.max(.02,normalized),1.42);var tipY=bottom-(bottom-top)*Math.max(.022,visualY);var points=[];for(var i=0;i<=110;i++){var t=i/110;var px=left+(tipX-left)*t;var py=bottom-(bottom-tipY)*Math.pow(t,1.95);points.push([px,py])}ctx.save();ctx.beginPath();ctx.moveTo(left,bottom);for(var a=0;a<points.length;a++)ctx.lineTo(points[a][0],points[a][1]);ctx.lineTo(tipX,h);ctx.lineTo(left,h);ctx.closePath();var shadow=ctx.createLinearGradient(0,top,0,h);shadow.addColorStop(0,ended?'rgba(255,125,145,.30)':'rgba(255,255,255,.31)');shadow.addColorStop(.42,ended?'rgba(255,125,145,.14)':'rgba(255,255,255,.145)');shadow.addColorStop(1,'rgba(255,255,255,0)');ctx.fillStyle=shadow;ctx.globalAlpha=.86;ctx.fill();ctx.restore();ctx.save();var line=ctx.createLinearGradient(left,bottom,tipX,tipY);line.addColorStop(0,'rgba(255,255,255,.32)');line.addColorStop(.55,'rgba(255,255,255,1)');line.addColorStop(1,ended?'rgba(255,125,145,.72)':'rgba(255,255,255,.64)');ctx.strokeStyle=line;ctx.lineWidth=4.2;ctx.lineCap='round';ctx.lineJoin='round';ctx.shadowColor='transparent';ctx.shadowBlur=0;ctx.beginPath();ctx.moveTo(left,bottom);for(var c=0;c<points.length;c++)ctx.lineTo(points[c][0],points[c][1]);ctx.stroke();ctx.restore();drawTip(ctx,tipX,tipY,ended);if(!ended)chartCurrent=visual}
  function renderHistory(now){var n=q('crashHistory');if(!n)return;var id=Math.floor(now/ROUND_MS);var html='';for(var i=1;i<=12;i++){html+='<span>'+fmt(roundStop(id-i))+'</span>'}n.innerHTML=html}
  function buttons(state){var s=q('crashStart'),c=q('crashCashout');var hasBet=activeBet&&activeBet.roundId===state.id&&!activeBet.settled;if(s){s.disabled=!!hasBet||state.crashed;s.textContent=hasBet?'Bet Placed':state.crashed?'Wait Next':'Place Bet'}if(c){c.disabled=!hasBet||state.crashed||activeBet.cashed;c.textContent=activeBet&&activeBet.cashed?'Cashed Out':'Cash Out'}}
  function placeBet(){var state=roundState(Date.now());if(state.crashed){show('Wait for the next round');return}if(activeBet&&activeBet.roundId===state.id&&!activeBet.settled){show('Bet already placed');return}var bet=normalizeAmount();if(balance()<bet){show('Not enough TON balance');return}change(-bet);activeBet={roundId:state.id,amount:bet,cashed:false,settled:false};show('Bet placed')}
  function cashout(){var state=roundState(Date.now());if(!activeBet||activeBet.roundId!==state.id||activeBet.settled||activeBet.cashed||state.crashed)return;activeBet.cashed=true;activeBet.settled=true;var back=Math.max(0,Math.floor(activeBet.amount*current));change(back);status('Cashed +' + toTon(back) + ' TON');show('Cashed at '+fmt(current));buttons(state)}
  function settleIfNeeded(state){if(activeBet&&activeBet.roundId!==state.id&&!activeBet.settled)activeBet.settled=true;if(activeBet&&activeBet.roundId===state.id&&state.crashed&&!activeBet.settled){activeBet.settled=true;status(activeBet.cashed?'Round ended':'Crashed')}if(settledRoundId!==state.id&&state.crashed){settledRoundId=state.id;renderHistory(Date.now())}}
  function update(){var now=Date.now();var state=roundState(now);if(currentRoundId!==state.id){currentRoundId=state.id;chartCurrent=1;current=1;lastProgress=0;status('Running');renderHistory(now)}var nextSec=(state.nextIn/1000).toFixed(1);nextLabel('Next round '+nextSec+'s');setCountdown(nextSec+'s',false);setTotal(state.elapsed/1000);if(state.crashed){current=state.stop;mult(current);drawGraph(lastProgress,true)}else{current=Math.min(state.stop,multAt(state.elapsed/1000));chartCurrent=current;mult(current);drawGraph(progressAt(state.elapsed),false)}settleIfNeeded(state);buttons(state);requestAnimationFrame(update)}
  ...
  function bind(){mult(1);status('Running');renderHistory(Date.now());var s=q('crashStart'),c=q('crashCashout'),input=q('crashAmount');if(s)s.onclick=placeBet;if(c)c.onclick=cashout;if(input)input.addEventListener('change',normalizeAmount);document.addEventListener('click',function(ev){var b=ev.target&&ev.target.closest&&ev.target.closest('button');if(!b)return;var a=b.getAttribute('data-action');if(a==='crash-half'){ev.preventDefault();half()}if(a==='crash-double'){ev.preventDefault();doubleAmount()}});window.addEventListener('resize',function(){drawGraph(lastProgress,roundState(Date.now()).crashed)});requestAnimationFrame(update)}
