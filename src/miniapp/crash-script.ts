...
(function(){
  ...
  function locateRound(now){
    ...
    var inCrashHold=!running&&waitElapsed<CRASH_HOLD_MS;
    return{id:c.id,start:scheduleCache.start,local:local,runElapsed:Math.min(local,c.runMs),waitElapsed:waitElapsed,stop:c.stop,runMs:c.runMs,running:running,waiting:!running,inCrashHold:inCrashHold,nextIn:nextIn}
  }
  ...
  function bind(){mult(1);status('Waiting');renderHistory(locateRound(Date.now()));var s=q('crashStart'),c=q('crashCashout'),input=q('crashAmount'),auto=q('crashAutoCashout');if(s)s.onclick=placeBet;if(c)c.onclick=cashout;if(input)input.addEventListener('change',normalizeAmount);if(auto)auto.addEventListener('change',normalizeAutoCashout);document.addEventListener('click',function(ev){var b=ev.target&&ev.target.closest&&ev.target.closest('button');if(!b)return;var a=b.getAttribute('data-action');if(a==='crash-half'){ev.preventDefault();half()}if(a==='crash-double'){ev.preventDefault();doubleAmount()}});window.addEventListener('resize',function(){var state=locateRound(Date.now());drawGraph(lastProgress,state.inCrashHold)});requestAnimationFrame(update)}
