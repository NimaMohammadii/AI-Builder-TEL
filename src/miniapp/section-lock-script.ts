...
(function(){
  ...
  function loadLocks(force){var now=Date.now();if(!force&&lastFullLoadAt&&now-lastFullLoadAt<FULL_RELOAD_COOLDOWN_MS){applyLocks();return Promise.resolve()}lastFullLoadAt=now;lastUserLoadAt=now;return Promise.all([loadGlobalLocks(),loadUserControls()]).then(applyLocks)}
