...
(function(){
  ...
  function renderMissions(day){
    ...
    box.innerHTML=list.map(function(m,i){
      ...
      var done=claimed.has(k),ready=claimable.has(k)&&!done&&!!id&&!!userId(),busy=claimBusy[k],pulse=claimPulse[k];
      var action=done?'<span class="daily-rewards-xp daily-rewards-claimed '+(pulse?'claim-success':'')+'">'+(pulse?'<i>✓</i><em>Claimed</em>':'Claimed')+'</span>':ready?'<button class="daily-rewards-xp daily-rewards-claim" type="button" data-action="claim-daily-reward" data-day="'+activeDay+'" data-mission-id="'+esc(id)+'" '+(busy?'disabled':'')+'>Claim</button>':'<span class="daily-rewards-xp daily-rewards-xp-static">+'+xp+' XP</span>';
  ...
  async function claim(day,missionId){var id=userId();if(!id)return;var k=key(day,missionId);if(claimBusy[k])return;claimBusy[k]=true;renderMissions(activeDay);try{var res=await fetch('/app/api/daily-rewards/claim',{method:'POST',credentials:'same-origin',headers:{'content-type':'application/json'},body:JSON.stringify({userId:id,day:day,missionId:missionId})});var json=await res.json();if(!res.ok)throw new Error(json.error||'Could not claim');if(!rewardsData)rewardsData={days:[],claimed:[]};if(!Array.isArray(rewardsData.claimed))rewardsData.claimed=[];if(json.claimedKey&&rewardsData.claimed.indexOf(json.claimedKey)===-1)rewardsData.claimed.push(json.claimedKey);if(json.profile)rewardsData.profile=json.profile;claimPulse[k]=true;try{window.dispatchEvent(new CustomEvent('vexa-xp-updated',{detail:{profile:json.profile,missionId:missionId,day:day,xp:json.xp}}))}catch(e){}renderMissions(activeDay);setTimeout(function(){delete claimPulse[k];refresh(activeDay,true)},1100)}catch(e){var status=q('dailyRewardsMissionCount');if(status)status.textContent=e.message||'Claim failed'}finally{delete claimBusy[k];renderMissions(activeDay)}}
