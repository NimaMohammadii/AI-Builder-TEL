...
(function(){
  ...
  ready(function(){
    ...
    var MARKETS={
      ...
      oil:{label:'Oil',question:'Oil in 30 days: up or down?',stream:'wss://fstream.asterdex.com/ws/clusdt@markPrice@1s',decimals:2,step:.05,symbol:'Oil'},
      gold:{label:'Gold',question:'Gold in 30 days: up or down?',stream:'wss://fstream.asterdex.com/ws/xauusdt@markPrice@1s',decimals:2,step:.5,symbol:'Au'}
    };
    ...
    function syncRound(my,id){
      ...
      return fetch('/app/api/predict-round?market='+encodeURIComponent(id)+'&userId='+encodeURIComponent(userId),{cache:'no-store',headers:headers}).then(function(r){return r.json().then(function(j){if(!r.ok)throw new Error((j&&j.error)||'Could not load prediction round');return j})}).then(function(d){
        ...
        if(countdown&&Number(round.remainingMs)>=0){var text=timeLeft(Number(round.remainingMs));if(countdown.textContent!==text)countdown.textContent=text}syncBetAvailability();syncTrend();updateEstimate();return true;
    ...
    function renderClock(){
      ...
      if(eventMode){
        ...
        remaining=Math.max(0,ends-now);if(roundLockDeadline&&now>=roundLockDeadline)syncBetAvailability();if(remaining<=0){syncRound(seq,market).then(function(ok){if(ok&&isActive()&&!eventMode)startClock()});return}var roundText=timeLeft(remaining);if(countdown&&countdown.textContent!==roundText)countdown.textContent=roundText;
      }
