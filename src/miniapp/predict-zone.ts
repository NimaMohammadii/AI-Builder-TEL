...
(function(){
  ...
  ready(function(){
    ...
    function syncBetAvailability(){var reason=currentBetBlockReason();root.querySelectorAll('button[data-predict-choice]').forEach(function(btn){btn.disabled=!!reason&&reason!=='Add GRAM to predict'});if(reason&&trendLabel)trendLabel.textContent=reason}
    ...
    function syncRound(my,id){
      ...
      return fetch('/app/api/predict-round?market='+encodeURIComponent(id)+'&userId='+encodeURIComponent(userId),{cache:'no-store',headers:headers}).then(function(r){return r.json().then(function(j){if(!r.ok)throw new Error((j&&j.error)||'Could not load prediction round');return j})}).then(function(d){
        ...
        var cachedGramUsd=readGramUsd();if(cachedGramUsd>0)gramUsd=cachedGramUsd;currentRound=round;roundLockDeadline=Date.now()+Math.max(0,Number(round.lockRemainingMs||0));updateBalance(d);renderHistory(round);if(Number(round.startPrice)>0){entry=Number(round.startPrice);if(start)start.textContent=formatPrice(entry)}
        ...
        if(market==='bitcoin'&&countdown&&Number(round.remainingMs)>=0){var text=timeLeft(Number(round.remainingMs));if(countdown.textContent!==text)countdown.textContent=text}syncBetAvailability();syncTrend();updateEstimate();loadGramUsd();return true;
    ...
    function openBet(nextSide){var blocked=currentBetBlockReason();if(blocked){if(blocked==='Add GRAM to predict'&&window.VexaCreditGuard&&typeof window.VexaCreditGuard.insufficient==='function')window.VexaCreditGuard.insufficient({source:'predict',message:'Insufficient balance'});if(trendLabel)trendLabel.textContent=blocked;return}side=eventMode?(nextSide==='no'?'no':'yes'):(nextSide==='down'?'down':'up');if(betInput)betInput.value='';loadGramUsd();updateEstimate();setStatus('','');if(betCloseTimer){clearTimeout(betCloseTimer);betCloseTimer=0}pauseChartDuringBetTransition();betOpen=true;if(card)card.classList.add('bet-mode');if(betStage)betStage.setAttribute('aria-hidden','false')}
