...
(function(){
  ...
  ready(function(){
    ...
    var sheetHome=null,sheetNext=null,sheetViewportHandler=null,sheetScrollTop=0,rootOverflow='',rootOverflowPriority='',rootOverscroll='',rootOverscrollPriority='';
    ...
    function positionBetSheet(){if(!sheet)return;var viewport=window.visualViewport,left=viewport?viewport.offsetLeft:0,top=viewport?viewport.offsetTop:0,width=viewport?viewport.width:window.innerWidth,height=viewport?viewport.height:window.innerHeight;sheet.style.left=Math.round(left)+'px';sheet.style.top=Math.round(top)+'px';sheet.style.width=Math.round(width)+'px';sheet.style.height=Math.round(height)+'px';root.scrollTop=sheetScrollTop}
    function mountBetSheet(){if(!sheet)return;sheetScrollTop=root.scrollTop;rootOverflow=root.style.getPropertyValue('overflow-y');rootOverflowPriority=root.style.getPropertyPriority('overflow-y');rootOverscroll=root.style.getPropertyValue('overscroll-behavior');rootOverscrollPriority=root.style.getPropertyPriority('overscroll-behavior');root.style.setProperty('overflow-y','hidden','important');root.style.setProperty('overscroll-behavior','none','important');if(sheet.parentNode!==document.body){sheetHome=sheet.parentNode;sheetNext=sheet.nextSibling;document.body.appendChild(sheet)}if(window.visualViewport&&!sheetViewportHandler){sheetViewportHandler=positionBetSheet;window.visualViewport.addEventListener('resize',sheetViewportHandler);window.visualViewport.addEventListener('scroll',sheetViewportHandler)}positionBetSheet()}
    function unmountBetSheet(){if(!sheet)return;if(window.visualViewport&&sheetViewportHandler){window.visualViewport.removeEventListener('resize',sheetViewportHandler);window.visualViewport.removeEventListener('scroll',sheetViewportHandler)}sheetViewportHandler=null;sheet.style.removeProperty('left');sheet.style.removeProperty('top');sheet.style.removeProperty('width');sheet.style.removeProperty('height');if(rootOverflow)root.style.setProperty('overflow-y',rootOverflow,rootOverflowPriority);else root.style.removeProperty('overflow-y');if(rootOverscroll)root.style.setProperty('overscroll-behavior',rootOverscroll,rootOverscrollPriority);else root.style.removeProperty('overscroll-behavior');root.scrollTop=sheetScrollTop;if(sheetHome&&sheet.parentNode!==sheetHome){if(sheetNext&&sheetNext.parentNode===sheetHome)sheetHome.insertBefore(sheet,sheetNext);else sheetHome.appendChild(sheet)}sheetHome=null;sheetNext=null}
    ...
    function animateChoice(choice){if(!choice)return;var symbol=choice.querySelector('.predict-zone-choice-symbol'),delta=choice.getAttribute('data-predict-choice')==='down'?4:-4;try{if(choice._vexaPredictTap&&choice._vexaPredictTap.cancel)choice._vexaPredictTap.cancel();if(choice.animate)choice._vexaPredictTap=choice.animate([{transform:'translate3d(0,0,0) scale(1)',filter:'brightness(1)'},{transform:'translate3d(0,1px,0) scale(.955)',filter:'brightness(1.12) saturate(1.06)',offset:.34},{transform:'translate3d(0,-1px,0) scale(1.018)',filter:'brightness(1.06)',offset:.68},{transform:'translate3d(0,0,0) scale(1)',filter:'brightness(1)'}],{duration:280,easing:'cubic-bezier(.18,.88,.24,1)'});if(symbol&&symbol.animate){if(symbol._vexaPredictTap&&symbol._vexaPredictTap.cancel)symbol._vexaPredictTap.cancel();symbol._vexaPredictTap=symbol.animate([{transform:'translate3d(0,0,0) scale(1)'},{transform:'translate3d(0,'+delta+'px,0) scale(1.14)',offset:.42},{transform:'translate3d(0,0,0) scale(1)'}],{duration:300,easing:'cubic-bezier(.18,.88,.24,1)'})}}catch(_){}}
    ...
    document.addEventListener('input',function(e){if(e.target===betInput)updateEstimate()},true);
    document.addEventListener('click',function(e){var target=e.target;if(!target||!(root.contains(target)||(sheet&&sheet.contains(target))))return;var preset=target.closest&&target.closest('[data-predict-bet-preset]');if(preset&&betInput){e.preventDefault();betInput.value=preset.getAttribute('data-predict-bet-preset')||'';updateEstimate();focusBetInput();return}var close=target.closest&&target.closest('[data-predict-bet-close]');if(close){e.preventDefault();closeBet();return}var choice=target.closest&&target.closest('[data-predict-choice]');if(choice){e.preventDefault();animateChoice(choice);openBet(choice.getAttribute('data-predict-choice'));return}var submit=target.closest&&target.closest('[data-predict-bet-submit]');if(submit&&sheet&&sheet.classList.contains('open')){e.preventDefault();submitBet()}},true);
    document.addEventListener('visibilitychange',function(){if(document.hidden){if(sheet&&sheet.classList.contains('open'))closeBet();if(raf){cancelAnimationFrame(raf);raf=0}closeFeed()}else resume()});
    ...
    window.VexaPredictBack=function(){if(sheet&&sheet.classList.contains('open')){closeBet();return true}return false};
