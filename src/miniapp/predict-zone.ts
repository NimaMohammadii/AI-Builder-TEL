...
(function(){
  ...
  ready(function(){
    ...
    function renderMarketPeriod(round){if(eventMode||market==='bitcoin'||!round)return;var month=roundMonthName(round);if(month&&question){var q=cfg().label+' in '+month+': up or down?';if(question.textContent!==q)question.textContent=q}if(month&&countdown&&countdown.textContent!==month)countdown.textContent=month;if(startLabel&&startLabel.textContent!=='Month start')startLabel.textContent='Month start'}
    ...
    function syncRound(my,id){
      ...
      return fetch('/app/api/predict-round?market='+encodeURIComponent(id)+'&userId='+encodeURIComponent(userId),{cache:'no-store',headers:headers}).then(function(r){return r.json().then(function(j){if(!r.ok)throw new Error((j&&j.error)||'Could not load prediction round');return j})}).then(function(d){
        ...
        currentRound=round;roundLockDeadline=Date.now()+Math.max(0,Number(round.lockRemainingMs||0));updateBalance(d);renderHistory(round);if(Number(round.startPrice)>0){entry=Number(round.startPrice);if(start)start.textContent=formatPrice(entry)}
