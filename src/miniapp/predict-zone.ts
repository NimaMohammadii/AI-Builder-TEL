...
(function(){
  ...
  ready(function(){
    ...
    function openBet(nextSide){side=nextSide==='down'?'down':'up';if(betInput)betInput.value='';updateEstimate();setStatus('','');if(betCloseTimer){clearTimeout(betCloseTimer);betCloseTimer=0}betOpen=true;if(card)card.classList.add('bet-mode');if(betStage)betStage.setAttribute('aria-hidden','false')}
