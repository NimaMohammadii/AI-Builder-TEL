export const PREDICT_ZONE_SECTION = `<section id="predictzone" class="view predict-zone-view">
  ...
  <script>(function(){
    ...
    function setupPredictChart(){
      ...
      function fallbackSeries(seed){prices=[];var step=seed>1000?1.8:.006;var waveSize=seed>1000?5:.018;for(var i=0;i<22;i++)prices.push(seed-step*12+i*step+Math.sin(i/2.8)*waveSize);currentPrice=seed+Math.sin(22/2.8)*waveSize;targetPrice=currentPrice;if(activeMarket==='ton')setTonUsdPrice(currentPrice);tailY=null;setAxisInstant(currentPrice);}
      function scaleInfo(delta){var m=market();var range=m.axisRange||50;if(!axisCenter)setAxisInstant(currentPrice||m.seed);if(currentPrice>axisTarget+range*.42||currentPrice<axisTarget-range*.42)setAxisTarget(currentPrice);easeAxis(delta||16);return {min:axisCenter-range/2,max:axisCenter+range/2};}
      function priceToY(value,scale){var y=padY+((scale.max-value)/(scale.max-scale.min))*(height-padY*2);return clamp(y,padY,height-padY);}
      ...
      if(window.MutationObserver){new MutationObserver(syncEngine).observe(root,{attributes:true,attributeFilter:['class']});}
    }
