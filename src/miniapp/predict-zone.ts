  ...
  <div class="predict-zone-simple-shell">
    ...
    <article class="predict-zone-glass-card predict-zone-btc-preview-card trend-flat" data-predict-card>
      ...
      <div class="predict-zone-bet-stage" data-predict-bet-stage aria-hidden="true">
        <label class="predict-zone-bet-input-wrap"><input class="predict-zone-bet-input" data-predict-bet-input type="number" min="0" step="0.01" inputmode="decimal" placeholder="0.00" /><span class="predict-zone-bet-estimate" data-predict-bet-estimate></span><span class="predict-zone-bet-side"><span class="predict-zone-bet-token">GRAM</span><span class="predict-zone-bet-usd" data-predict-bet-usd>≈ —</span></span></label>
...
@media(prefers-reduced-motion:reduce){.predict-zone-question-image,.predict-zone-live-price,.predict-zone-choice-symbol{animation:none!important}.predict-zone-category-card,.predict-zone-choice,.predict-zone-glass-card,.predict-zone-live-meta,.predict-zone-chart-preview,.predict-zone-bet-stage,.predict-zone-bet-presets button,.predict-zone-bet-submit,.predict-zone-chart-grid span,.predict-zone-price-axis span{transition:none!important}}
`;
...
(function(){
  ...
  ready(function(){
    ...
    var W=360,H=220,L=0,R=58,P=24,HISTORY=23,SAMPLE_MS=3000;
    ...
    function syncBetAvailability(){var reason=currentBetBlockReason();root.querySelectorAll('button[data-predict-choice]').forEach(function(btn){btn.disabled=!!reason});if(reason&&trendLabel)trendLabel.textContent=reason}
    ...
    function applyPrice(value,my,id){
      ...
      if(changed&&!betAnimating&&live){var liveText=formatPrice(p);if(live.textContent!==liveText)live.textContent=liveText}
    ...
    function syncRound(my,id){
      ...
      return fetch('/app/api/predict-round?market='+encodeURIComponent(id)+'&userId='+encodeURIComponent(userId),{cache:'no-store',headers:headers}).then(function(r){return r.json().then(function(j){if(!r.ok)throw new Error((j&&j.error)||'Could not load prediction round');return j})}).then(function(d){
        ...
        gramUsd=Number(d&&d.gramUsd)||0;currentRound=round;roundLockDeadline=Date.now()+Math.max(0,Number(round.lockRemainingMs||0));updateBalance(d);renderHistory(round);if(Number(round.startPrice)>0){entry=Number(round.startPrice);if(start)start.textContent=formatPrice(entry)}
        ...
        if(market==='bitcoin'&&countdown&&Number(round.remainingMs)>=0){var text=timeLeft(Number(round.remainingMs));if(countdown.textContent!==text)countdown.textContent=text}syncBetAvailability();syncTrend();updateEstimate();return true;
    ...
    function updateEstimate(){var amount=Number(betInput&&betInput.value||0);if(betUsd){betUsd.textContent=gramUsd>0?'≈ $'+Math.max(0,amount*gramUsd).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}):'≈ —'}if(betEstimate)betEstimate.textContent=amount>0?'+'+formatTon(estimate(amount)):''}
    ...
    function openBet(nextSide){var blocked=currentBetBlockReason();if(blocked){if(trendLabel)trendLabel.textContent=blocked;return}side=eventMode?(nextSide==='no'?'no':'yes'):(nextSide==='down'?'down':'up');if(betInput)betInput.value='';updateEstimate();setStatus('','');if(betCloseTimer){clearTimeout(betCloseTimer);betCloseTimer=0}pauseChartDuringBetTransition();betOpen=true;if(card)card.classList.add('bet-mode');if(betStage)betStage.setAttribute('aria-hidden','false')}
    ...
    function renderClock(){
      ...
      if(eventMode){
        ...
        remaining=Math.max(0,ends-now);if(roundLockDeadline&&now>=roundLockDeadline)syncBetAvailability();if(remaining<=0){syncRound(seq,market).then(function(ok){if(ok&&isActive()&&!eventMode)startClock()});return}if(market==='bitcoin'){var roundText=timeLeft(remaining);if(countdown&&countdown.textContent!==roundText)countdown.textContent=roundText}else renderMarketPeriod(currentRound);
      }
    ...
    function resume(){
      ...
      if(!runtimeStarted){runtimeStarted=true;loadImages();selectMarket('bitcoin');return}
