...
(function(){
  ...
  ready(function(){
    ...
    var MARKETS={
      ...
      gold:{label:'Gold',question:'Gold up or down?',stream:'paxgusdt@miniTicker',history:'https://fapi.binance.com/fapi/v1/aggTrades?symbol=PAXGUSDT&limit=23',wsBase:'wss://fstream.binance.com/ws/',decimals:2,step:.5,duration:300000,symbol:'Au'}
    };
    ...
    function seed(price){var i;values=(historyValues||[]).map(Number).filter(function(v){return isFinite(v)&&v>0}).slice(-HISTORY);if(!values.length){for(i=0;i<HISTORY;i++)values.push(price)}current=price;last=price;raw=price;scaleMin=0;scaleMax=0;lastPointAt=0}
    ...
    function pauseChartDuringBetTransition(){betAnimating=true;if(betAnimationTimer)clearTimeout(betAnimationTimer);betAnimationTimer=setTimeout(function(){betAnimating=false;betAnimationTimer=0},440)}
    ...
    function openBet(nextSide){side=nextSide==='down'?'down':'up';if(betInput)betInput.value='';updateEstimate();setStatus('','');if(betCloseTimer){clearTimeout(betCloseTimer);betCloseTimer=0}pauseChartDuringBetTransition();betOpen=true;if(card)card.classList.add('bet-mode');if(betStage)betStage.setAttribute('aria-hidden','false')}
    function closeBet(){if(!betOpen)return;pauseChartDuringBetTransition();betOpen=false;if(betInput&&document.activeElement===betInput)betInput.blur();var tg=window.Telegram&&window.Telegram.WebApp;if(tg&&typeof tg.hideKeyboard==='function'){try{tg.hideKeyboard()}catch(_){}}if(card)card.classList.remove('bet-mode');betCloseTimer=setTimeout(finishBetClose,440)}
