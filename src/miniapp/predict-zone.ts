export const PREDICT_ZONE_SECTION = `<section id="predictzone" class="view predict-zone-view">
  <div class="predict-zone-simple-shell">
    <nav class="predict-zone-category-menu" aria-label="Predict markets">
      ...
      <button type="button" class="predict-zone-category-card" data-vexa-predict-market="world"><span>World</span></button>
      <button type="button" class="predict-zone-category-card" data-vexa-predict-market="tech"><span>Tech / AI</span></button>
      <button type="button" class="predict-zone-category-card" data-vexa-predict-market="culture"><span>Culture</span></button>
    </nav>
...
.predict-zone-live-price.tick-up,.predict-zone-live-price.tick-down{animation:none!important}

.predict-zone-chart-preview{position:relative!important;width:100%!important;height:170px!important;border-radius:21px!important;background:rgba(0,0,0,.15)!important;overflow:hidden!important;margin:0 0 10px!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.055)!important;transform:translate3d(0,0,0) scale(1);transform-origin:center top}.predict-zone-chart-grid{position:absolute;inset:0 66px 0 0;background:linear-gradient(90deg,rgba(255,255,255,.035) 1px,transparent 1px);background-size:48px 100%;opacity:.44}.predict-zone-chart-grid span{position:absolute;left:0;right:0;height:1px;background:rgba(255,255,255,.035);transform:translateY(-50%);transition:none!important;will-change:auto}.predict-zone-price-axis{position:absolute;right:7px;top:0;bottom:0;width:58px;z-index:4;pointer-events:none}.predict-zone-price-axis span{position:absolute;right:0;transform:translateY(-50%);color:rgba(255,255,255,.54);font-size:12px;font-weight:760;letter-spacing:-.03em;text-align:right;white-space:nowrap;transition:none!important;will-change:auto}.predict-zone-chart-preview svg{position:absolute;inset:0;width:100%;height:100%;filter:none!important}.predict-zone-chart-fill{fill:url(#predictMainFill)}.predict-zone-chart-line{fill:none;stroke:url(#predictMainLine);stroke-width:3;stroke-linecap:round;stroke-linejoin:round}.predict-zone-chart-dot{position:absolute;left:0;top:0;width:7px;height:7px;border-radius:999px;background:#ff9a3e;box-shadow:0 0 0 5px rgba(255,154,62,.13),0 0 10px rgba(255,145,46,.32);transform:translate(-50%,-50%);animation:none!important;will-change:auto}.predict-zone-price-guide{position:absolute;left:0;right:58px;top:0;height:0;border-top:1px dashed rgba(255,255,255,.28);transform:translateY(-50%);pointer-events:none;opacity:.56}.predict-zone-start-guide{position:absolute;left:0;right:70px;height:0;border-top:1.5px dashed rgba(255,255,255,.46);z-index:4;opacity:0;pointer-events:none}.predict-zone-start-guide.show{opacity:1}.predict-zone-start-guide span{position:absolute;left:9px;top:-16px;font-size:9px;font-weight:850;letter-spacing:.10em;color:rgba(255,255,255,.58);text-transform:uppercase}.predict-zone-start-target{display:none!important}
.predict-zone-chart-loader{position:absolute;inset:0;z-index:8;display:grid;place-items:center;pointer-events:none;background:linear-gradient(180deg,rgba(0,0,0,.02),rgba(0,0,0,.12));opacity:1;transition:opacity .16s ease}.predict-zone-chart-loader span{width:42px;height:42px;border-radius:50%;border:2px solid rgba(255,255,255,.08);border-top-color:rgba(255,255,255,.75);border-right-color:rgba(255,255,255,.28);animation:none!important}.predict-zone-chart-preview.ready .predict-zone-chart-loader{opacity:0;visibility:hidden}
...
@media(max-width:380px){.predict-zone-view{padding-left:0!important;padding-right:0!important}#predictzone .predict-zone-category-menu{left:0!important;width:100%!important;padding-left:0!important;padding-right:0!important}.predict-zone-glass-card{height:364px!important;min-height:0!important;padding:10px 12px!important}.predict-zone-card-top{top:18px!important;right:12px!important}.predict-zone-countdown{font-size:17px!important}.predict-zone-hero{padding-right:72px}.predict-zone-question-row{font-size:17px!important}.predict-zone-question-image{flex-basis:38px;width:38px;height:38px}.predict-zone-live-meta>div{height:44px!important;padding:6px 8px!important}.predict-zone-live-meta strong{font-size:12px!important}.predict-zone-chart-preview{height:158px!important;border-radius:20px!important}}
...
(function(){
  ...
  ready(function(){
    ...
    var MARKETS={
      bitcoin:{label:'Bitcoin',question:'Bitcoin up or down?',stream:'btcusdt@miniTicker',history:'https://api.binance.com/api/v3/aggTrades?symbol=BTCUSDT&limit=23',decimals:0,step:5,duration:300000,symbol:'₿'},
      oil:{label:'Oil',question:'Oil in 72h up or down?',stream:'clusdt@miniTicker',history:'https://fapi.binance.com/fapi/v1/aggTrades?symbol=CLUSDT&limit=23',wsBase:'wss://fstream.binance.com/ws/',decimals:2,step:.05,duration:259200000,symbol:'Oil'},
      gold:{label:'Gold',question:'Gold up or down?',stream:'xautusdt@miniTicker',history:'https://api.binance.com/api/v3/aggTrades?symbol=XAUTUSDT&limit=23',decimals:2,step:.5,duration:300000,symbol:'Au'}
    };
    ...
    function loadPriceHistory(my,id){var c=MARKETS[id]||cfg();historyValues=[];if(!c.history)return Promise.resolve();return fetch(c.history,{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}).then(function(rows){if(my!==seq||id!==market)return;historyValues=(Array.isArray(rows)?rows:[]).map(function(row){return Number(row&&row.p)}).filter(function(v){return isFinite(v)&&v>0}).slice(-HISTORY);if(readyPrice&&historyValues.length){values=historyValues.slice(-HISTORY);queueDraw()}else if(!readyPrice&&historyValues.length)applyPrice(historyValues[historyValues.length-1],my,id)}).catch(function(){if(my===seq&&id===market)historyValues=[]})}
    ...
    function connectFeed(my,id){
      ...
      var c=MARKETS[id]||cfg();clearReconnect();
      if(!c.stream){if(currentRound&&Number(currentRound.startPrice)>0)applyPrice(Number(currentRound.startPrice),my,id);return}
      try{
        var socket=new WebSocket((c.wsBase||'wss://stream.binance.com:9443/ws/')+c.stream);ws=socket;
        socket.onmessage=function(e){if(my!==seq||id!==market||ws!==socket)return;try{var j=JSON.parse(e.data);if(applyPrice(j.c||j.p,my,id))reconnectDelay=6000}catch(_){}};
    ...
    function syncRound(my,id){var userId=uid(),initData=telegramInitData(),headers={};if(userId&&initData)headers['x-telegram-init-data']=initData;return fetch('/app/api/predict-round?market='+encodeURIComponent(id)+'&userId='+encodeURIComponent(userId),{cache:'no-store',headers:headers}).then(function(r){return r.json()}).then(function(d){if(my!==seq||id!==market||eventMode)return;var round=d&&d.round;if(!round)return;currentRound=round;updateBalance(d);renderHistory(round);var starts=Date.parse(round.startsAt||'');if(starts)slot=starts;if(Number(round.startPrice)>0){entry=Number(round.startPrice);if(start)start.textContent=formatPrice(entry)}var initialPrice=Number(round.livePrice||round.startPrice||0);if(!readyPrice&&initialPrice>0)applyPrice(initialPrice,my,id);if(countdown&&Number(round.remainingMs)>=0){var text=timeLeft(Number(round.remainingMs));if(countdown.textContent!==text)countdown.textContent=text}syncTrend();updateEstimate()}).catch(function(){})}
    ...
    function pauseChartDuringBetTransition(){betAnimating=true;cancelDraw();if(betAnimationTimer)clearTimeout(betAnimationTimer);betAnimationTimer=setTimeout(function(){betAnimating=false;betAnimationTimer=0;queueDraw()},440)}
    ...
    function openBet(nextSide){if(eventMode&&(!currentEvent||currentEvent.locked)){setStatus('This prediction is closed.','bad');return}side=eventMode?(nextSide==='no'?'no':'yes'):(nextSide==='down'?'down':'up');if(betInput)betInput.value='';updateEstimate();setStatus('','');if(betCloseTimer){clearTimeout(betCloseTimer);betCloseTimer=0}pauseChartDuringBetTransition();betOpen=true;if(card)card.classList.add('bet-mode');if(betStage)betStage.setAttribute('aria-hidden','false')}
    function closeBet(immediate){if(!betOpen&&!immediate)return;if(betInput&&document.activeElement===betInput)betInput.blur();var tg=window.Telegram&&window.Telegram.WebApp;if(tg&&typeof tg.hideKeyboard==='function'){try{tg.hideKeyboard()}catch(_){}}betOpen=false;if(immediate){if(betCloseTimer){clearTimeout(betCloseTimer);betCloseTimer=0}if(betAnimationTimer){clearTimeout(betAnimationTimer);betAnimationTimer=0}betAnimating=false;if(card)card.classList.remove('bet-mode');if(betStage)betStage.setAttribute('aria-hidden','true');return}pauseChartDuringBetTransition();if(card)card.classList.remove('bet-mode');betCloseTimer=setTimeout(finishBetClose,440)}
