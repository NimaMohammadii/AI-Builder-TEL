  ...
  ready(function(){
    ...
    function connectFeed(my,id){
      ...
      try{
        ...
        var socket=new WebSocket(protocol+'//'+location.host+'/app/api/predict-stream');ws=socket;
        socket.onopen=function(){if(my!==seq||id!==market||ws!==socket){try{socket.close()}catch(e){}return}socket.send(JSON.stringify({market:id,userId:userId,initData:initData}))};
        socket.onmessage=function(e){if(my!==seq||id!==market||ws!==socket)return;try{var j=JSON.parse(e.data);if(j&&j.type==='price'){if(applyPrice(j.price,my,id))reconnectDelay=6000}else if(j&&j.type==='error'&&trendLabel)trendLabel.textContent='Price feed unavailable'}catch(_){}};
    ...
    function resume(){
      ...
      if(wasSuspended){var resumedSlot=slotFor(Date.now());if(resumedSlot!==slot){slot=resumedSlot;entry=Number(raw||current||last||0)}var my=seq,id=market;syncRound(my,id).then(function(){if(my===seq&&id===market&&!eventMode&&isActive()&&!ws&&!reconnectTimer)connectFeed(my,id)});queueDraw()}
