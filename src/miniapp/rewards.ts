export const REWARDS_SECTION = `<section id="rewards" class="view rewards-view">
  <style>
    ...
    #rewards .rewards-hero-image-wrap{position:relative;width:min(44vw,170px);height:clamp(62px,20vw,104px);margin:calc(-62px + env(safe-area-inset-top)) auto 0;display:grid;place-items:center;pointer-events:none;animation:rewardsHeroFloat 5.8s ease-in-out infinite;will-change:transform;filter:drop-shadow(0 14px 22px rgba(0,0,0,.26))}
    ...
    #rewards .rewards-mission-list{width:100%!important;max-width:430px!important;margin:64px auto 0!important;display:grid!important;gap:10px!important;align-content:start!important;justify-items:stretch!important}
