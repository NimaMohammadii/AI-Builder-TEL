export const HOME_TWO_SECTION = `<section id="home" class="view active" data-home-variant="two">
  ...
  <style>
    ...
    #home[data-home-variant="two"] .home-two-reel-marker.is-hit:before{animation:homeTwoMarkerHit .34s cubic-bezier(.18,.88,.24,1)!important}
    @keyframes homeTwoMarkerHit{0%{transform:translateX(-50%) translateY(0) scale(1)}36%{transform:translateX(-50%) translateY(3px) scale(.88)}72%{transform:translateX(-50%) translateY(-1px) scale(1.08)}100%{transform:translateX(-50%) translateY(0) scale(1)}}
    ...
    #home[data-home-variant="two"] .home-two-reel-card.is-spinning .home-two-prize{filter:blur(.55px) brightness(.88)!important;opacity:.92!important;transform:translateZ(0) scale(.965)!important}
  ...
  (function(){
    ...
    function iconHtml(kind){
      if(kind==='ton')return '<span class="home-two-prize-icon"><img src="/app/api/credit-icon.png" alt="" decoding="async"></span>';
      return '<span class="home-two-prize-icon"><span class="home-two-ticket-icon" aria-hidden="true"><i></i></span></span>'
    }
    function prizeHtml(p){return '<div class="home-two-prize" data-reward-kind="'+p.kind+'" data-reward-amount="'+p.amount+'">'+iconHtml(p.kind)+'<strong>'+p.amount+'</strong><small>'+p.label+'</small></div>'}
    ...
    function build(){
      var html='';
      for(var i=0;i<74;i++)html+=prizeHtml(rewards[randomInt(rewards.length)]);
      track.innerHTML=html;
      var cards=track.children;
      var start=cards[Math.min(4,cards.length-1)];
      track.style.transition='none';
      track.style.transform='translate3d('+centerX(start)+'px,0,0)';
      target=null
    }
    function finish(){
      if(!busy)return;
    ...
    function spin(){
      ...
      build();
      var cards=track.children;
      var targetIndex=61+randomInt(7);
      ...
      track.style.transition='transform 5.7s cubic-bezier(.085,.67,.055,1)';
      track.style.transform='translate3d('+centerX(target)+'px,0,0)'
    }
    function init(){
      ...
      track.addEventListener('transitionend',function(e){if(e.propertyName==='transform')finish()});
