...
(function(){
  ...
  function focusAmountInput(anchor){var editor=document.querySelector('[data-plinko-bet-editor]'),input=document.querySelector('[data-plinko-bet-input]');if(!input)return;var rect=anchor&&anchor.getBoundingClientRect?anchor.getBoundingClientRect():null;if(editor&&rect)editor.style.setProperty('--plinko-editor-top',Math.max(120,rect.top+rect.height/2)+'px');if(editor)editor.classList.remove('active');input.value=money(currentBet());input.setAttribute('inputmode','decimal');input.setAttribute('autocomplete','off');input.setAttribute('enterkeyhint','done');try{input.focus({preventScroll:true});input.select()}catch(e){try{input.focus();input.select()}catch(_){}}
  }
  ...
  document.addEventListener('pointerdown',function(ev){var el=dropAssetFromEvent(ev);if(el)smoothPress(el)},{capture:true,passive:true});
  ...
  document.addEventListener('keydown',function(ev){var editor=document.querySelector('[data-plinko-bet-editor]');var input=document.querySelector('[data-plinko-bet-input]');if(input&&document.activeElement===input){if(ev.key==='Enter'){ev.preventDefault();input.blur();setBet(input.value)}if(ev.key==='Escape'){ev.preventDefault();input.blur();return}return}if(!editor||!editor.classList.contains('active'))return;if(ev.key==='Escape')closeEditor(false);if(ev.key==='Enter')closeEditor(true)});
  document.addEventListener('input',function(ev){if(ev.target&&ev.target.id==='plinkoBet')setBet(ev.target.value);if(ev.target&&ev.target.hasAttribute&&ev.target.hasAttribute('data-plinko-bet-input')){var input=q('plinkoBet'),open=document.querySelector('[data-plinko-bet-input-open]');if(input)input.value=ev.target.value;if(open)open.textContent=money(ev.target.value);renderStats()}});
