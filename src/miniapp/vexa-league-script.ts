...
(function(){
  ...
  function patchOverlayStyle(){
    var style=document.getElementById('vexaOverlayStyles');
    if(!style||!style.textContent)return;
    style.textContent=style.textContent
      .replace('.leaderboard-page .leaderboard-back,.leaderboard-page .top-players-hero-back{display:none!important}', '.leaderboard-page .leaderboard-back{display:none!important}')
      .replace('.leaderboard-page .leaderboard-back,.leaderboard-page .top-players-hero-back{display:none!important;}', '.leaderboard-page .leaderboard-back{display:none!important;}');
  }
  ...
  function withImageCacheBust(url){var base=String(url||'/app/api/top-players-hero-image');return base+(base.indexOf('?')>-1?'&':'?')+'v='+String(Date.now())}
  function heroImage(d){return withImageCacheBust(d&&d.topPlayersHeroImageUrl)}
  function heroImgHtml(d){return '<img src="'+esc(heroImage(d))+'" alt="" data-retry="0" onerror="var r=Number(this.dataset.retry||0);if(r<12){this.dataset.retry=String(r+1);var img=this;setTimeout(function(){img.style.display=\'block\';img.src=\'/app/api/top-players-hero-image?v=\'+Date.now()},900)}else{this.style.display=\'none\'}"/>'}
  ...
  function renderLoading(){ensureHeroStyle();var page=q('leaderboardPage');if(!page)return;var d=fallbackLeague();page.innerHTML='<section class="top-players-hero-card">'+backButtonHtml()+'<p class="top-players-hero-kicker">Top Players</p><h2 class="top-players-hero-title">Top 50 Players</h2><p class="top-players-hero-sub">Loading players...</p><div class="top-players-hero-art">'+heroImgHtml(d)+'</div></section><div class="missions-title" style="margin-top:18px"><strong>Top Players</strong><span>Loading</span></div><div class="leaderboard-list"><div class="mission-row"><span class="mission-icon">#</span><span class="mission-main"><strong>Loading players</strong><span>Preparing Top 50 list</span></span><span class="mission-reward">...</span></div></div>';ensureHeroStyle();syncNativeBack();}
