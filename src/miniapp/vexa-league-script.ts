...
(function(){
  ...
  async function refresh(force){updateHomeCard();var d=await loadLeague(Boolean(force));renderLeague(d)}
  window.VexaLeague={refresh:refresh,load:loadLeague};
  document.addEventListener('click',function(ev){var b=ev.target&&ev.target.closest&&ev.target.closest('button[data-action="open-leaderboard"]');if(b)setTimeout(function(){refresh(false)},80)},true);
