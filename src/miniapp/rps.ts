...
<section id="rps" class="view rps-view">
  <style>
    ...
    .rps-wrap{width:100%;max-width:560px;margin:0 auto;display:grid;gap:8px}
    ...
    .rps-multipliers.rps-win .rps-multiplier{color:rgba(220,255,232,.96);background:linear-gradient(180deg,rgba(8,86,42,.58),rgba(2,32,17,.72));box-shadow:inset 0 1px 0 rgba(149,255,187,.16),0 0 18px rgba(8,92,43,.26),0 12px 28px rgba(0,0,0,.36);animation:rpsMultiplierWin .82s cubic-bezier(.18,.86,.2,1) both}
    .rps-multipliers.rps-loss .rps-multiplier{color:rgba(255,218,226,.94);background:linear-gradient(180deg,rgba(85,5,28,.64),rgba(31,1,12,.82));box-shadow:inset 0 1px 0 rgba(255,124,151,.14),0 0 18px rgba(92,5,30,.28),0 12px 28px rgba(0,0,0,.38);animation:rpsMultiplierLoss .82s cubic-bezier(.18,.86,.2,1) both}
    .rps-arena{position:relative;min-height:468px;display:grid;grid-template-rows:auto auto auto auto;border:0!important;border-radius:0;background:transparent!important;box-shadow:none!important;overflow:visible!important;padding:4px 0 0}
    ...
    @media(max-width:380px){.rps-arena{min-height:428px}.rps-duel{margin:18px 0 12px}.rps-hand-card{height:148px}.rps-hand-img{width:86px;height:86px}.rps-hand-card.has-rps-image .rps-hand-img{width:104px;height:104px}.rps-choice{height:78px}.rps-choice .rps-hand-img{width:50px;height:50px}.rps-panel{margin-top:-6px}}
  </style>
  <div class="rps-wrap">
    <div class="rps-arena">
  ...
  <script>
    (function(){
      ...
      function flashMultipliers(type){if(!multWrap)return;multWrap.classList.remove('rps-win','rps-loss');void multWrap.offsetWidth;multWrap.classList.add(type==='win'?'rps-win':'rps-loss');setTimeout(function(){if(multWrap)multWrap.classList.remove('rps-win','rps-loss')},980)}
