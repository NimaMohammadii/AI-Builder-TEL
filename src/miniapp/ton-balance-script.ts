...
(function(){
  ...
  async function flushPending(force){var u=user();if(!u.id)return read();loadPending();if(syncing&&!force)return read();if(!pendingEvents.length)return read();var original=pendingEvents.slice();var legacyPlinko=original.filter(function(item){return item&&item.section==='plinko'});if(legacyPlinko.length){pendingEvents=original.filter(function(item){return !item||item.section!=='plinko'});savePending();if(!pendingEvents.length){try{var fresh=await fetchServerBalance();if(Number.isFinite(fresh))write(fresh,0,false)}catch(e){}return read()}}var initData=telegramInitData();if(!initData)return read();var events=pendingEvents.slice(0,100);var remainder=pendingEvents.slice(events.length);var next=events.reduce(function(sum,item){return sum+Math.floor(Number(item.deltaNano)||0)},0);pendingEvents=remainder;pendingDelta=pendingSum();savePending();syncing=true;try{var r=await fetch('/app/api/ton-balance/game-delta',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({userId:u.id,initData:initData,deltaNano:next,deltas:events}),keepalive:!!force});var j=await r.json().catch(function(){return null});if(!r.ok)throw new Error(j&&j.error?j.error:'Could not sync TON balance');var server=Number(j&&j.tonBalanceNano);if(Number.isFinite(server)&&!hasPending())write(server,0,false)}catch(e){loadPending();pendingEvents=events.concat(pendingEvents);pendingDelta=pendingSum();savePending()}syncing=false;if(hasPending())scheduleFlush(false);return read()}
  async function pushDelta(delta,section){var u=user();var value=Math.floor(Number(delta)||0);if(!value)return read();var before=read();var optimistic=write(before+value,value,false);if(!u.id)return optimistic;loadPending();pendingEvents.push({deltaNano:value,section:normalizeSection(section)||activeSection()});pendingDelta=pendingSum();savePending();scheduleFlush(false);return optimistic}
