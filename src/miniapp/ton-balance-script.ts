...
(function(){
  ...
  function pendingIsOnlyPlinko(){return pendingEvents.length&&pendingEvents.every(function(item){return item&&item.section==='plinko'})}
  function scheduleFlush(force){if(force){if(flushTimer){clearTimeout(flushTimer);flushTimer=0}return flushPending(true)}if(flushQueued||syncing||!hasPending())return read();if(pendingIsOnlyPlinko()){if(!flushTimer)flushTimer=setTimeout(function(){flushTimer=0;flushPending(false)},10000);return read()}flushQueued=true;queueMicrotask(function(){flushQueued=false;flushPending(false)});return read()}
  async function flushPending(force){if(flushTimer){clearTimeout(flushTimer);flushTimer=0}var u=user();if(!u.id)return read();loadPending();var events=pendingEvents.slice();var next=events.reduce(function(sum,item){return sum+Math.floor(Number(item.deltaNano)||0)},0);if(!next)return read();if(syncing&&!force)return read();pendingEvents=[];pendingDelta=0;savePending();syncing=true;try{var r=await fetch('/app/api/ton-balance/game-delta',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({userId:u.id,deltaNano:next,deltas:events}),keepalive:!!force});var j=await r.json().catch(function(){return null});var server=Number(j&&j.tonBalanceNano);if(Number.isFinite(server)&&!hasPending())write(server,0,false)}catch(e){loadPending();pendingEvents=events.concat(pendingEvents);pendingDelta=pendingSum();savePending()}syncing=false;if(hasPending())scheduleFlush(false);return read()}
