...
(function(){
  ...
  function normalizePendingEvents(items){
    var out=[];(Array.isArray(items)?items:[]).forEach(function(item){var delta=Math.floor(Number(item&&item.deltaNano));if(Number.isFinite(delta)&&delta)out.push({deltaNano:delta,section:normalizeSection(item&&item.section)||'unknown'})});return out
  }
  function pendingSum(){return pendingEvents.reduce(function(sum,item){return sum+Math.floor(Number(item.deltaNano)||0)},0)}
  function loadPending(){
    var u=user();
    if(!u.id)return 0;
    if(pendingUserId===u.id)return pendingSum();
    pendingUserId=u.id;
    try{
      var raw=localStorage.getItem(pendingKey())||'';
      var parsed=null;
      if(raw&&raw.charAt(0)==='{')parsed=JSON.parse(raw);
      if(raw&&!parsed){localStorage.removeItem(pendingKey());pendingEvents=[];return 0}
      var updatedAt=parsed?Number(parsed.updatedAt||0):0;
      if(!updatedAt||Date.now()-updatedAt>PENDING_MAX_AGE_MS){localStorage.removeItem(pendingKey());pendingEvents=[]}
      else pendingEvents=normalizePendingEvents(parsed.events||[{deltaNano:parsed.delta,section:parsed.section}]);
      pendingDelta=pendingSum();
    }catch(e){pendingEvents=[];pendingDelta=0}
    return pendingSum();
  }
  function savePending(){
    var key=pendingKey();
    if(!key)return;
    try{pendingDelta=pendingSum();if(pendingEvents.length)localStorage.setItem(key,JSON.stringify({events:pendingEvents,delta:pendingDelta,updatedAt:Date.now()}));else localStorage.removeItem(key)}catch(e){}
  }
  ...
  async function flushPending(force){
    ...
    var next=events.reduce(function(sum,item){return sum+Math.floor(Number(item.deltaNano)||0)},0);
    if(!next)return read();
    if(syncing&&!force)return read();
    pendingEvents=[];pendingDelta=0;
    savePending();
    syncing=true;
    try{
      var r=await fetch('/app/api/ton-balance/game-delta',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({userId:u.id,deltaNano:next,deltas:events}),keepalive:!!force});
      var j=await r.json().catch(function(){return null});
      var server=Number(j&&j.tonBalanceNano);
      if(Number.isFinite(server)&&!hasPending())write(server,0,false);
    }catch(e){
      loadPending();
      pendingEvents=events.concat(pendingEvents);pendingDelta=pendingSum();
      savePending();
    }
    syncing=false;
    return read();
