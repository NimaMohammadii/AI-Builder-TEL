...
(function(){
  ...
  function readDomBalance(){var nodes=document.querySelectorAll('[data-ton-balance-display],[data-credit-display],#topTonBalance,#plinkoCredit,#minesCredit,#creditCount,#plinkoCreditHeader');for(var i=0;i<nodes.length;i++){var raw=nodes[i].getAttribute('data-ton-balance-raw')||nodes[i].getAttribute('data-credit-raw');if(raw!==null){var r=clean(raw);if(Number.isFinite(r))return r}var n=parseTonText(nodes[i].textContent);if(Number.isFinite(n)&&n>=0)return n}return 0}
  ...
  function render(value){var balance=clean(value);localStorage.setItem(KEY,String(balance));document.querySelectorAll('[data-ton-balance-display],[data-credit-display],#topTonBalance,#plinkoCredit,#minesCredit,#creditCount,#plinkoCreditHeader').forEach(function(el){el.setAttribute('data-ton-balance-raw',String(balance));el.setAttribute('data-credit-raw',String(balance));el.textContent=formatTon(balance)});return balance}
  ...
  async function load(){var u=user();if(!u.id){write(read(),0,true);return read()}try{var r=await fetch('/app/api/activity',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({userId:u.id,username:u.username,firstName:u.firstName,section:'games'})});var j=await r.json().catch(function(){return null});var next=Number(j&&(j.tonBalanceNano!==undefined?j.tonBalanceNano:j.credit));if(j&&j.ok&&Number.isFinite(next)){return write(next,0,false)}}catch(e){}write(read(),0,true);return read()}
  ...
  window.addEventListener('vexa-credit-sync',function(ev){if(!ev||!ev.detail)return;var balance=Number(ev.detail.credit);if(Number.isFinite(balance))render(balance)});
