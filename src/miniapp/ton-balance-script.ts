...
(function(){
  ...
  function loadPending(){var u=user();if(!u.id)return 0;if(pendingUserId===u.id)return pendingSum();pendingUserId=u.id;pendingEvents=[];pendingDelta=0;var key=PENDING_PREFIX+u.id;try{var raw=localStorage.getItem(key)||'';if(!raw)return 0;var parsed=null;if(raw.charAt(0)==='{')parsed=JSON.parse(raw);else{var legacyDelta=Math.floor(Number(raw));if(Number.isFinite(legacyDelta)&&legacyDelta)parsed={delta:legacyDelta,section:'unknown'}}if(!parsed||typeof parsed!=='object'){pendingUserId='';return 0}var source=Array.isArray(parsed.events)?parsed.events:[{eventId:parsed.eventId,deltaNano:parsed.delta,section:parsed.section}];pendingEvents=normalizePendingEvents(source);pendingDelta=pendingSum();if(pendingEvents.length)localStorage.setItem(key,JSON.stringify({events:pendingEvents,delta:pendingDelta,updatedAt:Number(parsed.updatedAt)||Date.now()}));else localStorage.removeItem(key)}catch(e){pendingEvents=[];pendingDelta=0;pendingUserId=''}return pendingSum()}
