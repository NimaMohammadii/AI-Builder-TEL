...
(function(){
  ...
  document.addEventListener('visibilitychange',function(){if(document.hidden){smartTick(true);savePlayMs();clearPlayTimer()}else{exitDrained=false;lastTickAt=Date.now();refreshFromUserIntent(false);load({force:false})}});
  window.addEventListener('focus',function(){exitDrained=false;lastTickAt=Date.now();refreshFromUserIntent(false);load({force:false})});
