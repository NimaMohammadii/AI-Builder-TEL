...
(function(){
  ...
  var rankImageCache={};
  var rankClickDelegateReady=false;
  ...
  function currentFallbackProfile(){var levelText='1';try{var line=document.getElementById('userLine');var m=String(line&&line.textContent||'').match(/Level\s+(\d+)/i);if(m&&m[1])levelText=m[1]}catch(e){}var level=Math.max(1,Math.floor(Number(levelText)||1));return{level:level,xp:0,totalXp:0,nextLevelXp:need(level),progressPercent:0,xpLeft:need(level),rankName:rank(level)}}
  ...
  function renderRankModal(){
    var p=clean(profile||currentFallbackProfile());
  ...
  function bindRankModalTrigger(img){var brand=document.querySelector('.brand');if(!rankClickDelegateReady){rankClickDelegateReady=true;document.addEventListener('click',function(e){var target=e.target&&e.target.closest&&e.target.closest('.brand');if(!target)return;e.preventDefault();e.stopPropagation();openRankModal()},true);document.addEventListener('touchend',function(e){var target=e.target&&e.target.closest&&e.target.closest('.brand');if(!target)return;e.preventDefault();e.stopPropagation();openRankModal()},true)}if(brand&&!brand.dataset.rankModalReady){brand.dataset.rankModalReady='1';brand.setAttribute('role','button');brand.setAttribute('tabindex','0');brand.setAttribute('aria-label','Open rank system');brand.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();openRankModal()}})}if(!img||img.dataset.rankModalReady==='1')return;img.dataset.rankModalReady='1';img.setAttribute('role','button');img.setAttribute('tabindex','0');img.setAttribute('aria-label','Open rank system')}
  function ensureRankClickReady(){bindRankModalTrigger(document.querySelector('.brand img.logo'))}
  ...
  function load(){var userId=id();ensureRankClickReady();if(!userId){render(currentFallbackProfile());return}loadPlayMs();fetch('/app/api/level?userId='+encodeURIComponent(userId)).then(function(r){return r.json()}).then(function(p){render(p);awardDailyOpen()}).catch(function(){render(currentFallbackProfile());awardDailyOpen()})}
  function tickPlayXp(){
    var now=Date.now();
  ...
  document.addEventListener('visibilitychange',function(){lastTickAt=Date.now();if(!document.hidden){markActivity();awardDailyOpen();ensureRankClickReady()}});
  ...
  setTimeout(ensureRankClickReady,250);
