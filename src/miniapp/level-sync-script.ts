...
(function(){
  ...
  function bindRankModalTrigger(img){if(!img)return;if(!rankClickDelegateReady){rankClickDelegateReady=true;document.addEventListener('click',function(e){var target=e.target&&e.target.closest&&e.target.closest('.brand img.logo');if(!target)return;e.preventDefault();e.stopPropagation();openRankModal()},true);document.addEventListener('touchend',function(e){var target=e.target&&e.target.closest&&e.target.closest('.brand img.logo');if(!target)return;e.preventDefault();e.stopPropagation();openRankModal()},true)}if(img.dataset.rankModalReady==='1')return;img.dataset.rankModalReady='1';img.setAttribute('role','button');img.setAttribute('tabindex','0');img.setAttribute('aria-label','Open rank system');img.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();openRankModal()});img.addEventListener('keydown',function(e){if(e.key==='Enter'||e.key===' '){e.preventDefault();openRankModal()}})}
  function ensureRankClickReady(){var img=document.querySelector('.brand img.logo');if(img)bindRankModalTrigger(img)}
