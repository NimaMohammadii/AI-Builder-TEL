...
(function(){
  ...
  function flushPendingXp(){var userId=id();if(flushTimer){clearTimeout(flushTimer);flushTimer=0}if(!userId||flushingXp)return;var pending=loadPendingXp();if(!pending.length)return;flushingXp=true;queue=queue.then(function(){function step(){var list=loadPendingXp();if(!list.length)return Promise.resolve();var ev=list[0];return fetch('/app/api/level/xp',{method:'POST',cache:'no-store',headers:{'content-type':'application/json','cache-control':'no-store'},body:xpBody(ev)}).then(function(r){if(!r.ok)throw new Error('xp sync failed');return r.json().catch(function(){return null})}).then(function(j){list=loadPendingXp();if(list.length&&list[0].eventId===ev.eventId)list.shift();else list=list.filter(function(x){return x.eventId!==ev.eventId});savePendingXp(list);if(j&&j.profile)render(j.profile,{authoritative:true,force:true});if(j&&j.leveledUp&&j.profile)popup(j.profile.level,j.profile.rankName);return step()})}return step()}).catch(function(){}).then(function(){flushingXp=false})}
  function scheduleFlushPendingXp(force){if(force){flushPendingXp();return}if(flushTimer||flushingXp||!loadPendingXp().length)return;flushTimer=setTimeout(function(){flushTimer=0;flushPendingXp()},FLUSH_DEBOUNCE_MS)}
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){observedSection=section();load({force:true});markActivity();lastTickAt=Date.now();schedulePlayTimer()});else{observedSection=section();load({force:true});markActivity();lastTickAt=Date.now();schedulePlayTimer()}
})();
`;