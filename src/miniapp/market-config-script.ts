...
(function(){
  ...
  function priceIcon(){return '<img class="market-price-icon" src="'+NFT_PRICE_ICON_URL+'?v='+(window.__vexaNftPriceIconVersion||'1')+'" alt="TON" decoding="async"/>'}
  ...
  async function openDetail(item){
    ...
    if(isTelegram){var priceBox=sheet.querySelector('.market-detail-price strong');if(priceBox){var old=priceBox.querySelector('.market-price-icon');if(!old)priceBox.insertAdjacentHTML('afterbegin',priceIcon());else old.src=NFT_PRICE_ICON_URL+'?v='+(window.__vexaNftPriceIconVersion||'1')}}
    if(buy){buy.remove()}
