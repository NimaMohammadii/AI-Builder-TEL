...
(function(){
  ...
  function firstTonPrice(raw){raw=esc(raw);var m=raw.match(/([0-9]+(?:[.,][0-9]+)?)\s*(?:TON|Ton|ton)\b/);if(m&&m[1])return m[1].replace(',','.');return ''}
  function telegramMeta(item){var raw=esc(item&&item.description||'')+' '+esc(item&&item.utility||'')+' '+esc(item&&item.supply||'')+' '+esc(item&&item.price||'');var num=(raw.match(/#\s*\d+/)||[])[0]||'';var price=firstTonPrice(raw);return {number:num.replace(/\s+/g,''),price:price.trim()}}
  ...
  function priceButton(value){return '<span class="market-price-button vexa-fragment-price"><img src="'+TON_ICON+'" alt="TON" decoding="async"/><b>'+esc(value||'0')+'</b></span>'}
