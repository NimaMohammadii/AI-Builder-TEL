...
#predictzone .predict-crypto-card{position:relative;width:100%;height:calc((100vw - (var(--predict-card-gap) * 2)) / 3 + (var(--predict-card-gap) * 2))!important;min-height:128px!important;max-height:180px!important;border:0!important;border-radius:22px!important;background:transparent!important;box-shadow:none!important;-webkit-backdrop-filter:none!important;backdrop-filter:none!important;display:grid;grid-template-columns:78px minmax(0,1fr) auto;align-items:center;gap:12px;padding:0 6px!important;text-align:left;color:#fff;overflow:hidden;transition:transform .18s ease!important}
...
#predictzone .predict-crypto-price{height:34px;min-width:74px;border-radius:999px;background:rgba(255,255,255,.055);box-shadow:inset 0 1px 0 rgba(255,255,255,.10);display:inline-flex;align-items:center;justify-content:center;padding:0 10px;margin-right:6px!important;color:rgba(255,255,255,.86);font-size:12px;font-weight:850;letter-spacing:-.035em;white-space:nowrap}
