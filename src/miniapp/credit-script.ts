...
(function(){
  ...
  function emit(credit,delta){try{window.dispatchEvent(new CustomEvent('vexa-credit-sync',{detail:{credit:credit,delta:Math.floor(Number(delta)||0),display:formatTon(credit),rate:NANO_PER_TON}}))}catch(e){}}
  ...
  window.VexaCredit={read:read,write:write,add:add,render:function(){return render(read())},load:load,format:formatTon,rate:NANO_PER_TON,parse:parseTonText};
