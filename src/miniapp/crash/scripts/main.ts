...
(function(){
  ...
  function stepAuto(delta){var input=q('crashAutoCashout');if(!input)return;var currentValue=Number(String(input.value||'').replace(/,/g,'.'));if(!Number.isFinite(currentValue)||currentValue<1.01)currentValue=2;var next=Math.max(1.01,Math.min(200,currentValue+Number(delta||0)));input.value=next.toFixed(2);normalizeAutoCashout()}
