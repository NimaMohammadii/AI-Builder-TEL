...
(function(){
  ...
  function cashout(){var state=locateRound(Date.now());if(!activeBet||activeBet.roundId!==state.id||activeBet.settled||activeBet.cashed||state.waiting)return;activeBet.cashed=true;activeBet.settled=true;var m=current,back=Math.max(0,Math.floor(activeBet.amount*m));activeBet.payoutNano=back;change(back);status('Cashed +' + toTon(back) + ' GRAM');show('Cashed at '+fmt(m));window.dispatchEvent(new CustomEvent('vexa-crash-cashout',{detail:{roundId:state.id,multiplier:m,payoutNano:back}}));buttons(state);scheduleUpdate(0)}
