...
#crash .crash-rocket-flight{--rocket-thrust:.3;--rocket-angle:70deg;position:absolute;left:50%;top:50%;width:min(56vw,220px);height:min(56vw,220px);transform:translate3d(-50%,-50%,0) rotate(var(--rocket-angle)) scale(.78);transform-origin:50% 50%;will-change:transform,opacity;animation:crashRocketDrift 3.6s ease-in-out infinite;transition:opacity .22s ease;opacity:1}
...
@keyframes crashRocketDrift{0%,100%{transform:translate3d(calc(-50% - 2px),calc(-50% + 2px),0) rotate(var(--rocket-angle)) scale(.78)}50%{transform:translate3d(calc(-50% + 2px),calc(-50% - 2px),0) rotate(var(--rocket-angle)) scale(.78)}}
