...
(function(){
  ...
  async function waitForTonConnectionRestore(ui){var restored=ui&&ui.connectionRestored;if(!restored||typeof restored.then!=='function')return usableWalletConnection(ui);var ok=await restored;if(ok&&usableWalletConnection(ui))return true;if(ui&&ui.connected)await clearStaleWalletSession(ui);return false}
