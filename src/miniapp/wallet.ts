export const WALLET_SECTION = `<div id="wallet" class="wallet-sheet" aria-hidden="true">
  <style>
    ...
    body.wallet-open.deposit-open #depositSheet .deposit-pay-button,body.wallet-open.withdraw-open #withdrawSheet .deposit-pay-button{width:min(100%,340px)!important;height:44px!important;min-height:44px!important;border:0!important;border-radius:17px!important;background:linear-gradient(135deg,rgba(92,14,36,.84),rgba(45,6,19,.88))!important;color:#fff!important;font-size:13px!important;font-weight:950!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.09),inset 0 -1px 0 rgba(255,255,255,.055),0 12px 24px rgba(0,0,0,.20)!important;backdrop-filter:blur(10px) saturate(1.12)!important;-webkit-backdrop-filter:blur(10px) saturate(1.12)!important}
...
(function(){
  ...
  function setFinanceCopy(){var dep=document.querySelector('#depositSheet .deposit-copy:not(.transactions-copy)');var wit=document.querySelector('#withdrawSheet .deposit-copy');var lang=supportedLang(financeCopyLang);if(dep){dep.textContent=(financeCopy.deposit[lang]||financeCopy.deposit.en);applyTextDirection(dep,lang)}if(wit){wit.textContent=(financeCopy.withdraw[lang]||financeCopy.withdraw.en);applyTextDirection(wit,lang)}applyWithdrawGramLabels()}
  ...
  function closeDepositSheet(){var button=document.querySelector('#depositSheet .deposit-close');if(button)button.click()}
  ...
  async function confirmTonPayment(){if(paying)return;var input=ensureTonInput();var raw=twoDecimalText(input&&input.value);var amount=parseTon(raw);if(!amount){toast('Enter a valid TON amount');setStatus('Enter a valid TON amount','error');return}if(amount<MIN_TON_DEPOSIT){toast('Minimum deposit is '+MIN_TON_DEPOSIT+' TON');setStatus('Minimum deposit is '+MIN_TON_DEPOSIT+' TON','error');return}if(input)input.value=raw;paying=true;try{setStatus('Preparing payment…','pending');var ui=await loadTonConnect();applyTonConnectDarkTheme();var deposit=await createDeposit(raw);savePending(deposit);var nano=deposit.amountNano||tonToNanoString(deposit.amountTon||raw);var wallet=deposit.wallet;closeDepositSheet();await new Promise(function(resolve){setTimeout(resolve,180)});await chooseTonWallet(ui);await ui.sendTransaction({validUntil:Math.floor(Date.now()/1000)+300,messages:[{address:wallet,amount:String(nano),payload:tonCommentPayload(deposit.id)}]});setStatus('Payment sent. Checking confirmation once…','pending');await verifyDeposit(deposit.id)}catch(error){setStatus(error&&error.message?error.message:'Payment cancelled or failed','error');toast(error&&error.message?error.message:'Payment cancelled or failed')}paying=false}
  ...
  function bind(){ensureUi();loadFinanceCopy();document.addEventListener('input',function(ev){if(ev.target&&['starsAmountSheet','tonAmountSheet'].indexOf(ev.target.id)!==-1)syncModeUi()});document.addEventListener('click',function(ev){var button=ev.target&&ev.target.closest?ev.target.closest('button'):null;if(!button)return;var action=button.getAttribute('data-action');if(action==='close-wallet'){closeWallet();return}if(action==='open-deposit'){ensureWallet();loadFinanceCopy();setTimeout(ensureUi,40);verifyPendingDeposit();if(depositMode==='ton')loadTonUsd()}if(action==='open-withdraw'){loadFinanceCopy()}if(action==='set-deposit-mode'){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation&&ev.stopImmediatePropagation();setMode(button.getAttribute('data-mode'))}if(action==='confirm-ton-payment'){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation&&ev.stopImmediatePropagation();confirmTonPayment()}},true);document.addEventListener('visibilitychange',function(){if(!document.hidden)syncOnAppEvent()});window.addEventListener('focus',syncOnAppEvent);window.addEventListener('online',syncOnAppEvent);setTimeout(ensureUi,220);setTimeout(ensureUi,900)}
