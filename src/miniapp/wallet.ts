...
(function(){
  ...
  function selectDepositMethod(mode){ensureUi();setMode(mode);setDepositNav(true)}
  ...
  function bind(){ensureUi();loadFinanceCopy();setDepositNav(false);document.addEventListener('input',function(ev){if(ev.target&&['starsAmountSheet','tonAmountSheet'].indexOf(ev.target.id)!==-1)syncModeUi()});document.addEventListener('click',function(ev){var button=ev.target&&ev.target.closest?ev.target.closest('button'):null;if(!button)return;var action=button.getAttribute('data-action');if(action==='close-wallet'){closeWallet();return}if(action==='open-deposit'){ensureWallet();setDepositNav(false);loadFinanceCopy();setTimeout(ensureUi,40);verifyPendingDeposit();if(depositMode==='ton')loadTonUsd()}if(action==='open-withdraw'){loadFinanceCopy()}if(action==='confirm-ton-payment'){ev.preventDefault();ev.stopPropagation();ev.stopImmediatePropagation&&ev.stopImmediatePropagation();confirmTonPayment()}},true);document.addEventListener('visibilitychange',function(){if(!document.hidden)syncOnAppEvent()});window.addEventListener('focus',syncOnAppEvent);window.addEventListener('online',syncOnAppEvent);setTimeout(ensureUi,220);setTimeout(ensureUi,900)}
