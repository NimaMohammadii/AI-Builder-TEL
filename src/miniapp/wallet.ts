...
(function(){
  ...
  function loadStarsGramRate(force){var now=Date.now();if(!force&&starsGramPerStar&&now-starsGramRateLoadedAt<60000)return Promise.resolve(starsGramPerStar);if(starsGramRatePromise)return starsGramRatePromise;var user=ownerId();if(!user)return Promise.resolve(0);starsGramRatePromise=fetch('/app/api/stars/deposits?userId='+encodeURIComponent(user),{headers:{accept:'application/json'},cache:'no-store'}).then(function(r){return r.json().then(function(data){if(!r.ok)throw new Error(data&&data.error||'Stars rate request failed');return data})}).then(function(data){var rate=data&&data.rate;var value=Number(rate&&rate.gramPerStar);if(!Number.isFinite(value)||value<=0)throw new Error('Invalid Stars rate');starsGramPerStar=value;starsGramRateLoadedAt=Date.now();syncModeUi();return value}).catch(function(){return starsGramPerStar||0}).finally(function(){starsGramRatePromise=null});return starsGramRatePromise}
