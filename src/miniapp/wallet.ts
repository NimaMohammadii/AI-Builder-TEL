...
(function(){
  ...
  function mainnetWalletAccount(ui){if(!ui||!ui.connected)return null;var account=walletAccount(ui);if(!account)throw new Error('Wallet account is unavailable');if(String(account.chain||'')!==TON_MAINNET)throw new Error('Switch your wallet to TON Mainnet');return account}
  ...
  function waitForTonConnectionRestore(ui){var restored=ui&&ui.connectionRestored;return restored&&typeof restored.then==='function'?restored.then(function(){return true}):Promise.resolve(true)}
  ...
  function syncTonConnectionUi(ui){if(!tonFlowActive)return false;if(!ui||!ui.connected){showTonConnectGate(false,'');return false}try{mainnetWalletAccount(ui);showTonPaymentForm();return true}catch(error){showTonConnectGate(false,error&&error.message?error.message:'Switch your wallet to TON Mainnet');return false}}
  function bindTonStatus(ui){if(tonStatusBound||!ui||typeof ui.onStatusChange!=='function')return;tonStatusBound=true;ui.onStatusChange(function(){if(!tonConnectionReady||!tonFlowActive)return;syncTonConnectionUi(ui)})}
  ...
  async function confirmTonPayment(){if(paying)return;var ui,account;try{ui=await loadTonConnect();bindTonStatus(ui);await waitForTonConnectionRestore(ui);tonConnectionReady=true;account=mainnetWalletAccount(ui)}catch(error){tonFlowActive=true;showTonConnectGate(false,error&&error.message?error.message:'Could not load wallet connection');setDepositNav(true);return}if(!account){tonFlowActive=true;showTonConnectGate(false,'Connect your wallet to continue');setDepositNav(true);return}var input=ensureTonInput();var raw=twoDecimalText(input&&input.value);var amount=parseTon(raw);if(!amount){toast('Enter a valid TON amount');setStatus('Enter a valid TON amount','error');return}if(amount<MIN_TON_DEPOSIT){toast('Minimum deposit is '+MIN_TON_DEPOSIT+' TON');setStatus('Minimum deposit is '+MIN_TON_DEPOSIT+' TON','error');return}if(input)input.value=raw;paying=true;try{setStatus('Preparing payment…','pending');var deposit=await createDeposit(raw,account);savePending(deposit);var nano=deposit.amountNano||tonToNanoString(deposit.amountTon||raw);var wallet=deposit.wallet;var payload=String(deposit.payload||tonCommentPayload(deposit.id));await ui.sendTransaction({validUntil:Math.floor(Date.now()/1000)+300,network:TON_MAINNET,from:String(account.address),messages:[{address:wallet,amount:String(nano),payload:payload}]});setStatus('Payment sent. Checking confirmation…','pending');await verifyDeposit(deposit.id)}catch(error){setStatus(error&&error.message?error.message:'Payment cancelled or failed','error');toast(error&&error.message?error.message:'Payment cancelled or failed')}paying=false}
