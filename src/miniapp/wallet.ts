...
(function(){
  ...
  function syncModeUi(){var out=q('starsTonEquivalent');if(!out)return;if(depositMode==='ton'){var amount=parseTon(currentTonText());var price=readTonUsd();if(!price&&amount)loadTonUsd();var usd=price&&amount?amount*price:0;out.textContent='≈ $'+usd.toFixed(2);out.classList.add('usd-mode')}else{var s=starsInput();var stars=Math.max(0,Math.floor(Number(normalizeNumber(s&&s.value))||0));out.textContent=starsEquivalent(stars);out.classList.remove('usd-mode')}}
