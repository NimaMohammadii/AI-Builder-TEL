...
(function(){
  ...
  async function tonWalletSessionId(ui){if(!ui||typeof ui.getSessionId!=='function')return '';try{return String(await ui.getSessionId()||'').trim()}catch(e){return ''}}
  ...
  async function waitForTonConnectionRestore(ui){if(!tonConnectionReady){var restored=ui&&ui.connectionRestored;if(restored&&typeof restored.then==='function')await restored}return restoreVerifiedWalletSession(ui)}
