...
(function(){
  ...
  async function clearStaleWalletSession(ui){tonBridgeSessionVerified=false;if(!ui||!ui.connected||typeof ui.disconnect!=='function')return;try{await ui.disconnect()}catch(e){}}
  ...
  function installDoneOverlayStyle(){if(q('vexa-payment-done-style'))return;var style=document.createElement('style');style.id='vexa-payment-done-style';style.textContent='.vexa-payment-done{position:fixed!important;inset:0!important;z-index:2147483000!important;display:grid!important;place-items:center!important;pointer-events:none!important;opacity:0!important;transition:opacity .28s ease!important}.vexa-payment-done.show{opacity:1!important}.vexa-payment-done-card{position:relative!important;isolation:isolate!important;overflow:hidden!important;width:160px!important;height:160px!important;border:1px solid rgba(255,255,255,.16)!important;border-radius:40px!important;background:linear-gradient(145deg,rgba(255,255,255,.075),rgba(255,255,255,.018) 48%,rgba(255,255,255,.045))!important;backdrop-filter:blur(18px) saturate(1.06)!important;-webkit-backdrop-filter:blur(18px) saturate(1.06)!important;box-shadow:0 24px 72px rgba(0,0,0,.32),inset 0 1px 0 rgba(255,255,255,.22),inset 0 -1px 0 rgba(255,255,255,.055)!important;display:grid!important;place-items:center!important;transform:translateY(12px) scale(.94)!important;transition:transform .56s cubic-bezier(.16,1,.3,1),opacity .28s ease!important;opacity:0!important}.vexa-payment-done-card:before{content:""!important;position:absolute!important;z-index:0!important;inset:-60% -95%!important;background:linear-gradient(112deg,transparent 43%,rgba(255,255,255,.14) 50%,transparent 57%)!important;transform:translateX(-32%) rotate(7deg)!important;opacity:0!important}.vexa-payment-done-card>div{position:relative!important;z-index:1!important;text-align:center!important}.vexa-payment-done.show .vexa-payment-done-card{transform:translateY(0) scale(1)!important;opacity:1!important}.vexa-payment-done.show .vexa-payment-done-card:before{animation:vexaDoneGlassSweep .95s cubic-bezier(.2,.75,.2,1) .16s both!important}.vexa-payment-done-icon{position:relative!important;width:66px!important;height:66px!important;margin:auto!important;border:1px solid rgba(255,255,255,.14)!important;border-radius:999px!important;background:rgba(255,255,255,.025)!important;box-shadow:inset 0 1px 0 rgba(255,255,255,.17),0 12px 28px rgba(0,0,0,.14)!important;color:#64d98b!important;display:grid!important;place-items:center!important;animation:vexaDonePop .66s cubic-bezier(.16,1,.3,1) both!important}.vexa-payment-done-icon:after{content:""!important;position:absolute!important;inset:-1px!important;border:1px solid rgba(100,217,139,.22)!important;border-radius:inherit!important;opacity:0!important;transform:scale(.82)!important;animation:vexaDoneRing .58s cubic-bezier(.16,1,.3,1) .12s forwards!important}.vexa-payment-done-icon svg{width:35px!important;height:35px!important}.vexa-payment-done-icon path{stroke:currentColor!important;stroke-width:4.4px!important;stroke-dasharray:64!important;stroke-dashoffset:64!important;filter:none!important;animation:vexaDoneDraw .5s cubic-bezier(.22,.78,.26,1) .2s forwards!important}.vexa-payment-done-text{margin-top:13px!important;color:rgba(255,255,255,.86)!important;font-size:15px!important;font-weight:760!important;letter-spacing:.015em!important;text-shadow:none!important;opacity:0!important;transform:translateY(5px)!important;animation:vexaDoneText .36s ease .38s forwards!important}@keyframes vexaDoneGlassSweep{0%{opacity:0;transform:translateX(-34%) rotate(7deg)}38%{opacity:1}100%{opacity:0;transform:translateX(34%) rotate(7deg)}}@keyframes vexaDoneDraw{to{stroke-dashoffset:0}}@keyframes vexaDonePop{0%{transform:translateY(7px) scale(.76);opacity:0}62%{transform:translateY(0) scale(1.035);opacity:1}100%{transform:translateY(0) scale(1);opacity:1}}@keyframes vexaDoneRing{to{opacity:1;transform:scale(1)}}@keyframes vexaDoneText{to{opacity:1;transform:translateY(0)}}@media (prefers-reduced-motion:reduce){.vexa-payment-done,.vexa-payment-done-card{transition-duration:.01ms!important}.vexa-payment-done-card:before,.vexa-payment-done-icon,.vexa-payment-done-icon:after,.vexa-payment-done-icon path,.vexa-payment-done-text{animation-duration:.01ms!important;animation-delay:0ms!important}}';document.head.appendChild(style)}
  function showPaymentDone(){installDoneOverlayStyle();var old=q('vexaPaymentDone');if(old)old.remove();var overlay=document.createElement('div');overlay.id='vexaPaymentDone';overlay.className='vexa-payment-done';overlay.innerHTML='<div class="vexa-payment-done-card"><div><div class="vexa-payment-done-icon"><svg viewBox="0 0 52 52" fill="none"><path d="M15 27.5L23 35L38 17" stroke="currentColor" stroke-width="4.4" stroke-linecap="round" stroke-linejoin="round"/></svg></div><div class="vexa-payment-done-text">Done</div></div></div>';document.body.appendChild(overlay);requestAnimationFrame(function(){overlay.classList.add('show')});setTimeout(function(){overlay.classList.remove('show')},1850);setTimeout(function(){try{overlay.remove()}catch(e){}},2300)}
  ...
  function bindTonStatus(ui){if(tonStatusBound||!ui||typeof ui.onStatusChange!=='function')return;tonStatusBound=true;ui.onStatusChange(function(wallet){if(!wallet)tonBridgeSessionVerified=false;if(!tonConnectionReady||!tonFlowActive)return;syncTonConnectionUi(ui)},function(error){if(!tonConnectionReady||!tonFlowActive)return;if(isUnknownAppError(error)){clearStaleWalletSession(ui).then(function(){showTonConnectGate(false,'Wallet session expired. Connect again.')});return}showTonConnectGate(false,error&&error.message?error.message:'Wallet connection failed')})}
