...
(function(){
  ...
  var prizePoolAnimation=null,displayedPrizePoolNano=0,displayedPrizePoolReady=false,displayedPrizePoolRoundId='';
  ...
  function animatePrizePool(target){var value=q('#homeDrawInfoCard [data-prize-pool]');target=Math.max(0,Number(target)||0);if(prizePoolAnimation){try{prizePoolAnimation.cancel()}catch(e){}prizePoolAnimation=null}displayedPrizePoolNano=target;setPrizePoolText(target);if(!value||document.hidden||!q('#home.active')||typeof value.animate!=='function')return;try{if(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches)return}catch(e){}try{var animation=value.animate([{transform:'translate3d(0,-82%,0)',opacity:.12,offset:0},{transform:'translate3d(0,5%,0)',opacity:1,offset:.76},{transform:'translate3d(0,0,0)',opacity:1,offset:1}],{duration:420,easing:'cubic-bezier(.16,1,.3,1)',fill:'none'});prizePoolAnimation=animation;var done=function(){if(prizePoolAnimation===animation)prizePoolAnimation=null};animation.onfinish=done;animation.oncancel=done}catch(e){prizePoolAnimation=null}}
  function renderPrizePool(){var icon=q('#homeDrawInfoCard [data-prize-pool-icon]'),target=prizePoolNano(),roundId=String(state&&state.round&&state.round.id||'');if(!displayedPrizePoolReady||displayedPrizePoolRoundId!==roundId){if(prizePoolAnimation){try{prizePoolAnimation.cancel()}catch(e){}prizePoolAnimation=null}displayedPrizePoolReady=true;displayedPrizePoolRoundId=roundId;displayedPrizePoolNano=target;setPrizePoolText(target)}else if(Math.abs(target-displayedPrizePoolNano)>.5)animatePrizePool(target);if(icon){var src=balanceIconSrc();if(src){if(icon.getAttribute('src')!==src)icon.setAttribute('src',src);icon.style.display='block'}else icon.style.display='none'}}
