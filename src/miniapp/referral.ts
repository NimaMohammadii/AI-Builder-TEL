export const REFERRAL_SECTION = `<section id="referral" class="view referral-view">
  ...
  (function(){
    ...
    function shareFallback(link,text){var share='https://t.me/share/url?url='+encodeURIComponent(link)+'&text='+encodeURIComponent(text||'Join me on Vexa');try{if(tg&&typeof tg.openTelegramLink==='function'){tg.openTelegramLink(share);return}}catch(e){}location.href=share}
    ...
    function shareReferral(){var id=uid();if(!id)return;var link=linkFor(id);setShareText('Preparing');api('/app/api/referral/share',{method:'POST',body:JSON.stringify({userId:id,name:name()})}).then(function(data){var invite=data.inviteUrl||link;if(data.preparedMessageId&&tg&&typeof tg.shareMessage==='function'){try{var sent=tg.shareMessage(data.preparedMessageId);if(sent&&typeof sent.then==='function')sent.catch(function(){shareFallback(invite,data.fallbackText)});setShareText('Choose Chat');return}catch(e){}}shareFallback(invite,data.fallbackText)}).catch(function(){shareFallback(link,'Join me on Vexa')})}
