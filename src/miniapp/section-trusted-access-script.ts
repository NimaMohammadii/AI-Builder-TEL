...
(function(){
  ...
  var uid=String(user.id||localStorage.getItem('ownerId')||'').trim();
  var trusted=false;
  ...
  function readTrusted(){
    ...
    return nativeFetch('/app/api/user-access-override?userId='+encodeURIComponent(uid),{cache:'no-store'}).then(function(r){return r.json()}).then(function(data){trusted=!!(data&&data.trustedAccess);window.VexaTrustedAccess=trusted;if(trusted)removeLockViews();return trusted}).catch(function(){return false});
  }
  ...
  document.addEventListener('click',function(){if(trusted)setTimeout(removeLockViews,80)},true);
