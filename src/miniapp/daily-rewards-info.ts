export const DAILY_REWARDS_INFO_SECTION = `<section id="dailyrewardsinfo" class="view daily-rewards-info-view">
  <style>
    ...
    .daily-info-list{position:relative;min-height:0;display:grid;gap:32px;align-content:start;overflow-y:auto!important;overflow-x:hidden!important;-webkit-overflow-scrolling:touch;overscroll-behavior-y:contain;scrollbar-width:none;padding:0 0 92px!important;mask-image:linear-gradient(to bottom,#000 0,#000 calc(100% - 58px),rgba(0,0,0,0));-webkit-mask-image:linear-gradient(to bottom,#000 0,#000 calc(100% - 58px),rgba(0,0,0,0))}.daily-info-list::-webkit-scrollbar{display:none}
    .daily-info-row{display:grid;grid-template-columns:158px minmax(0,1fr);align-items:start;gap:14px;min-height:184px;padding:0 13px 18px 0;border-radius:26px;background:rgba(255,255,255,.04);box-shadow:inset 0 1px 0 rgba(255,255,255,.08),0 14px 28px rgba(0,0,0,.12);backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px);overflow:hidden}
