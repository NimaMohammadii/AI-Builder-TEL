...
#home .daily-rewards-banner-frame:before{content:"";position:absolute;left:0;top:0;width:100%;height:46%;pointer-events:none;z-index:2;background:linear-gradient(to bottom,rgba(0,0,0,.50) 0%,rgba(0,0,0,.28) 42%,rgba(0,0,0,0) 100%)}
...
@media(max-width:380px){#home .daily-rewards-drop-banner{left:60%;top:calc(-18px + env(safe-area-inset-top));width:136px;height:178px;transform:translate3d(-42%,0,0)}#home .daily-rewards-drop-banner.can-claim:active{transform:translate3d(-42%,0,0) scale(.985)}}
`;