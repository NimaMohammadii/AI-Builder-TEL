...
(function(){
  ...
  var NFT_KEY='vexaPlayZoneMixedNfts:v2';
  ...
  function loadLowNfts(force){if(nftBusy)return;var cached=readNftCache();if(cached&&Array.isArray(cached.items)&&cached.items.length){lastGoodNfts=cached.items;renderNfts(cached.items,true);if(!force&&Date.now()-Number(cached.ts||0)<180000)return}nftBusy=true;var ts=Date.now();Promise.all([
    ...
    if(items.length){writeNftCache(items);renderNfts(items,true)}else{renderNfts([],false)}
  }).catch(function(){renderNfts(lastGoodNfts,true)}).finally(function(){nftBusy=false})}
