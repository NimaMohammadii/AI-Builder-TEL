...
(function(){
  ...
  var dailyInfoToday=0;
  var dailyInfoStateReady=false;
  var homeScrollTop=0;
  ...
  function tgUserId(){var tg=window.Telegram&&window.Telegram.WebApp;var u=tg&&tg.initDataUnsafe&&tg.initDataUnsafe.user;return String((u&&u.id)||localStorage.getItem('ownerId')||'').trim()}
  ...
  function loadDailyInfoState(){if(dailyInfoStateReady)return Promise.resolve(dailyInfoToday);var id=tgUserId();var url='/app/api/daily-rewards'+(id?'?userId='+encodeURIComponent(id):'');return fetch(url,{credentials:'same-origin',cache:'no-store'}).then(function(r){return r.ok?r.json():null}).then(function(json){var t=Number(json&&json.today);if(Number.isFinite(t)&&t>=0&&t<7)dailyInfoToday=t;dailyInfoStateReady=true;return dailyInfoToday}).catch(function(){dailyInfoStateReady=true;return dailyInfoToday})}
  ...
  function liveBadge(i){return i===dailyInfoToday?'<span class="daily-info-live-dot" aria-label="Live day"></span>':''}
  function renderDailyInfo(){var box=document.getElementById('dailyInfoList');if(!box)return;box.innerHTML=dailyInfoRows.map(function(row,i){var day=i+1;return '<div class="daily-info-row '+(i===dailyInfoToday?'today':'')+'"><div class="daily-info-img"><img src="/app/api/daily-rewards-day-image/'+i+'" alt="" decoding="async" loading="lazy"></div><div class="daily-info-main"><span class="daily-info-day-wrap"><em class="daily-info-day">Day '+day+'</em>'+liveBadge(i)+'</span><b>'+row.title+'</b><small>'+dailyInfoText(row)+'</small></div></div>'}).join('');dailyInfoRendered=true}
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){mountDailyInfo();Promise.all([loadDailyInfoLang(),loadDailyInfoState()]).then(renderDailyInfo)});else{mountDailyInfo();Promise.all([loadDailyInfoLang(),loadDailyInfoState()]).then(renderDailyInfo)}
})();
`;
