...
(function(){
  ...
  async function trigger(eventId){if(eventId!=='admin_message')return;try{var message=await fetchMessage(eventId);if(message)enqueue(message)}catch(e){}}
  ...
  function boot(){
    ...
    window.VexaVoiceMessage={admin:function(){trigger('admin_message')}};
