...
(function(){
  ...
  ready(function(){
    ...
    function setLocked(locked){root.classList.toggle('predict-candle-locked',!!locked);root.querySelectorAll('[data-predict-choice]').forEach(function(b){b.disabled=false})}
    function candleTimerHtml(){setLocked(false);return fmt(candleEndMs()-Date.now())}
