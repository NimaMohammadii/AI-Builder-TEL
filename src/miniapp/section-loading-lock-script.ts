...
(function(){
  ...
  function style(){
    ...
    s.id='slmcss';
    s.textContent='.section-loading-mode{position:absolute;inset:0;z-index:120;display:grid;place-items:center;background:#000;color:#fff}.section-loading-mode-box{width:220px;display:grid;gap:20px;justify-items:center}.section-loading-mode-spin{width:66px;height:66px;border-radius:50%;border:9px solid rgba(255,255,255,.14);border-top-color:#fff;border-right-color:rgba(255,255,255,.58);animation:slmspin 2.8s linear infinite;box-sizing:border-box}.section-loading-mode-line{width:100%;height:4px;background:rgba(255,255,255,.12);overflow:hidden;border-radius:99px}.section-loading-mode-line i{display:block;height:100%;width:0;background:#fff;animation:slmbar 1800s linear forwards}@keyframes slmspin{to{transform:rotate(360deg)}}@keyframes slmbar{0%{width:0}100%{width:90%}}';
