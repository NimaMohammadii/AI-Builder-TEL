...
(function(){
  ...
  ready(function(){
    ...
    if(!document.getElementById('predictZoneToolbarStyles')){
      ...
      style.textContent='#predictzone .predict-toolbar-row{display:flex;align-items:center;justify-content:space-between;gap:10px;margin:-10px 0 6px;padding:0 4px}#predictzone .predict-mode-wrap{position:relative;width:134px;flex:0 0 134px;min-width:0;box-sizing:border-box}#predictzone .predict-mode-card,#predictzone .predict-credit-card{width:134px;height:42px;min-width:0;border:0;border-radius:999px;background:transparent;color:#fff;display:flex;align-items:center;justify-content:center;gap:8px;padding:0 16px;-webkit-backdrop-filter:blur(2px);backdrop-filter:blur(2px);box-shadow:0 12px 26px rgba(0,0,0,.14),inset 0 1px 0 rgba(255,255,255,.10),inset 0 -1px 0 rgba(255,255,255,.035)}#predictzone .predict-mode-card{position:relative;justify-content:flex-start;text-align:left;padding-right:34px;background:rgba(255,255,255,.055);box-sizing:border-box}#predictzone .predict-mode-card span{display:none}#predictzone .predict-mode-card strong{max-width:86px;color:#fff;font-size:13px;font-weight:820;letter-spacing:-.025em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height:1}#predictzone .predict-mode-card:after{content:"⌄";position:absolute;right:13px;top:50%;transform:translateY(-50%);color:rgba(255,255,255,.98);font-size:27px;font-weight:760;line-height:1;text-align:center;text-shadow:0 1px 8px rgba(0,0,0,.38)}#predictzone .predict-credit-card{flex:0 0 134px;justify-content:flex-end;background:rgba(255,255,255,.055)}#predictzone .predict-credit-card .ton-mini-icon{width:24px;height:24px}#predictzone .predict-credit-card .ton-mini-icon img{width:24px;height:24px;display:block;object-fit:contain;border:0;background:transparent;box-shadow:none}#predictzone .predict-credit-card b{color:#fff;font-size:13px;font-weight:820;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;letter-spacing:-.025em;font-variant-numeric:tabular-nums lining-nums;line-height:1}#predictzone .predict-mode-menu{position:absolute;left:0;right:0;top:48px;z-index:18;padding:6px;border-radius:20px;border:0;background:rgba(8,8,8,.94);box-shadow:0 12px 26px rgba(0,0,0,.24),inset 0 1px 0 rgba(255,255,255,.10),inset 0 -1px 0 rgba(255,255,255,.035);-webkit-backdrop-filter:blur(2px);backdrop-filter:blur(2px);opacity:0;transform:translateY(-6px) scale(.97);pointer-events:none;transition:opacity .16s ease,transform .16s ease}#predictzone .predict-mode-wrap.open .predict-mode-menu{opacity:1;transform:translateY(0) scale(1);pointer-events:auto}#predictzone .predict-mode-option{width:100%;height:34px;border-radius:15px;background:transparent;color:rgba(255,255,255,.68);font-size:12px;font-weight:760;text-align:left;padding:0 10px}#predictzone .predict-mode-option.active{background:rgba(255,255,255,.12);color:#fff}@media(max-width:380px){#predictzone .predict-toolbar-row{padding:0 2px;margin:-11px 0 5px}#predictzone .predict-mode-wrap{width:126px;flex-basis:126px;box-sizing:border-box}#predictzone .predict-mode-card,#predictzone .predict-credit-card{width:126px;flex-basis:126px}#predictzone .predict-mode-card,#predictzone .predict-credit-card{height:40px;padding:0 14px}#predictzone .predict-mode-card{padding-right:32px;box-sizing:border-box}#predictzone .predict-mode-card strong,#predictzone .predict-credit-card b{font-size:12.5px}#predictzone .predict-mode-card:after{right:11px;top:50%;font-size:26px}}';
