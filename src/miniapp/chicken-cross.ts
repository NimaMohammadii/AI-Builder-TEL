...
body:has(#hilo.active) .content{overflow:hidden!important}
#hilo{--cc-accent:#f3b85b;position:relative!important;height:100%!important;min-height:0!important;margin:0 -16px -8px!important;width:calc(100% + 32px)!important;padding:0 0 calc(24px + env(safe-area-inset-bottom))!important;overflow-y:auto!important;overflow-x:hidden!important;-webkit-overflow-scrolling:touch!important;overscroll-behavior-y:contain!important;touch-action:pan-y!important;scrollbar-width:none!important;background:#0d0f10!important}
...
(function(){
  function init(){
    ...
    function createEngine(THREE){
      ...
      renderer.setPixelRatio(Math.min(2,window.devicePixelRatio||1));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.06;
      var scene=new THREE.Scene();scene.background=new THREE.Color(0x121617);scene.fog=new THREE.FogExp2(0x121617,.024);
      ...
      var hemi=new THREE.HemisphereLight(0xcbd8df,0x263028,1.55);scene.add(hemi);
      var key=new THREE.DirectionalLight(0xfff3db,2.8);key.position.set(-7,14,10);key.castShadow=true;key.shadow.mapSize.set(1024,1024);key.shadow.camera.left=-13;key.shadow.camera.right=13;key.shadow.camera.top=18;key.shadow.camera.bottom=-8;key.shadow.camera.near=1;key.shadow.camera.far=55;key.shadow.bias=-.00035;scene.add(key,key.target);
      var rim=new THREE.DirectionalLight(0x7890a0,1.25);rim.position.set(9,5,-14);scene.add(rim);
      var streetGlow=new THREE.PointLight(0xffc56d,13,20,2);streetGlow.position.set(0,4,-8);scene.add(streetGlow);
      ...
      function makeRoadTexture(){var c=document.createElement('canvas');c.width=256;c.height=512;var x=c.getContext('2d'),image=x.createImageData(c.width,c.height);for(var p=0;p<image.data.length;p+=4){var grain=25+Math.floor(Math.random()*18);image.data[p]=grain;image.data[p+1]=grain+2;image.data[p+2]=grain+3;image.data[p+3]=255}x.putImageData(image,0,0);x.globalAlpha=.16;for(var n=0;n<90;n++){x.fillStyle=n%4?'#73777a':'#adb0ad';x.fillRect(Math.random()*256,Math.random()*512,1+Math.random()*2,7+Math.random()*28)}var t=new THREE.CanvasTexture(c);t.wrapS=t.wrapT=THREE.RepeatWrapping;t.repeat.set(5,18);t.colorSpace=THREE.SRGBColorSpace;t.anisotropy=Math.min(8,renderer.capabilities.getMaxAnisotropy());return t}
      var roadTexture=makeRoadTexture(),roadMat=mat(0x2a2d2f,.76,.04);roadMat.map=roadTexture;roadMat.bumpMap=roadTexture;roadMat.bumpScale=.035;roadMat.needsUpdate=true;var lineMat=mat(0xe4dfcf,.62,.04),curbMat=mat(0xaaa79e,.82,.02),edgeMat=mat(0x4d5352,.86,.02);
      ...
      var shoulderGeo=new THREE.BoxGeometry(2.8,.22,82),leftShoulder=new THREE.Mesh(shoulderGeo,edgeMat),rightShoulder=leftShoulder.clone();leftShoulder.position.set(-13.8,.04,-31);rightShoulder.position.set(13.8,.04,-31);leftShoulder.receiveShadow=rightShoulder.receiveShadow=true;scene.add(leftShoulder,rightShoulder);
      var curbGeo=new THREE.BoxGeometry(.72,.30,.62);for(var curbIndex=0;curbIndex<126;curbIndex++){var curbZ=1.1-curbIndex*.64;[-12.55,12.55].forEach(function(curbX,sideIndex){var curb=new THREE.Mesh(curbGeo,curbIndex%2?curbMat:edgeMat);curb.position.set(curbX,.15,curbZ);curb.castShadow=true;curb.receiveShadow=true;scene.add(curb)})}
      ...
      for(var b=0;b<24;b++){var side=b%2?1:-1,w=2.4+(b%4)*.55,h=2.8+(b*7%9)*.72,d=2.2+(b%3)*.8;var building=new THREE.Mesh(new THREE.BoxGeometry(w,h,d),buildingMat[b%3]);building.position.set(side*(16.2+(b%4)*1.2),h/2-.08,-(b*1.16)-4);building.castShadow=true;building.receiveShadow=true;scene.add(building)}
      ...
      function addTrafficLight(x,z,mirror){var signal=new THREE.Group(),pole=mesh(new THREE.CylinderGeometry(.055,.075,2.15,8),poleMat,0,1.07,0),box=mesh(new THREE.BoxGeometry(.34,.86,.30),mat(0x202526,.72,.18),0,2.03,0),offMat=mat(0x392f22,.65,.04),greenMat=mat(0x62b779,.36,.06,0x28643a);signal.add(pole,box);[-.25,0,.25].forEach(function(y,index){var bulb=mesh(new THREE.SphereGeometry(.075,10,8),index===2?greenMat:offMat,mirror?-.18:.18,2.03+y,.01);signal.add(bulb)});signal.position.set(x,0,z);scene.add(signal)}
      for(var streetIndex=0;streetIndex<14;streetIndex++){var streetZ=-2-streetIndex*5.2;addTree(-14.3,streetZ,streetIndex%2);addTree(14.3,streetZ-2.3,(streetIndex+1)%2);if(streetIndex%2===0){addStreetLight(-12.95,streetZ-1.2,false);addStreetLight(12.95,streetZ-3.5,true)}if(streetIndex%4===0){addTrafficLight(-12.9,streetZ-.2,false);addTrafficLight(12.9,streetZ-2.3,true)}}
      var crossingGeo=new THREE.BoxGeometry(1.25,.028,.22);for(var crossingLane=5;crossingLane<25;crossingLane+=6){for(var crossingX=-9;crossingX<=9;crossingX+=2.2){var stripe=new THREE.Mesh(crossingGeo,lineMat);stripe.position.set(crossingX,.035,-crossingLane*LANE+.55);stripe.receiveShadow=true;scene.add(stripe)}}
      ...
      function animate(){requestAnimationFrame(animate);if(!active||document.hidden)return;var dt=Math.min(.033,clock.getDelta()),time=performance.now();shownStep+=(targetStep-shownStep)*(1-Math.pow(.0008,dt));var chickenZ=1.2-shownStep*LANE;chicken.position.z=chickenZ;
        ...
        var desiredZ=chickenZ+14.5;camera.position.z+=(desiredZ-camera.position.z)*(1-Math.pow(.012,dt));camera.position.x+=(Math.sin(time*.00036)*.28-camera.position.x)*(1-Math.pow(.05,dt));camera.lookAt(0,1.05,chickenZ-5.8);streetGlow.position.z=chickenZ-5;key.position.z=chickenZ+9;key.target.position.set(0,0,chickenZ-5);renderer.render(scene,camera)}
