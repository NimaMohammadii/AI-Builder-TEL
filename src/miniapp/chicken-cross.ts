...
(function(){
  function init(){
    ...
    function createEngine(THREE){
      ...
      var camera=new THREE.PerspectiveCamera(42,1,.1,170);camera.position.set(CHICKEN_START_X,5.6,14.2);camera.lookAt(CHICKEN_START_X,1.0,CHICKEN_Z-4.6);
      var clock=new THREE.Clock(),active=true,targetStep=0,shownStep=0,maxVisualSteps=24,jumpStart=0,impactStart=0,impactResolve=null,impactContact=false;
      ...
      var streetGlow=new THREE.PointLight(0xffc56d,11,23,2);streetGlow.position.set(CHICKEN_START_X,4,-12);scene.add(streetGlow);
      ...
      function hit(value,maxSteps){maxVisualSteps=Math.max(1,Number(maxSteps)||maxVisualSteps);targetStep=Math.max(0,Number(value)||0);jumpStart=performance.now();impactStart=performance.now();impactContact=false;impactCar.visible=true;impactCar.position.set(stepX(targetStep,maxVisualSteps),.02,-38);return new Promise(function(resolve){impactResolve=resolve})}
      function reset(){targetStep=0;shownStep=0;maxVisualSteps=modes[mode]?modes[mode].steps:24;jumpStart=performance.now();impactStart=0;impactContact=false;if(impactResolve){impactResolve();impactResolve=null}impactCar.visible=false;camera.position.x=CHICKEN_START_X}
      ...
      function animate(){requestAnimationFrame(animate);if(!active||document.hidden)return;var dt=Math.min(.033,clock.getDelta()),time=performance.now();shownStep+=(targetStep-shownStep)*(1-Math.pow(.0008,dt));var chickenX=stepX(shownStep,maxVisualSteps);chicken.position.x=chickenX;chicken.position.z=CHICKEN_Z;
        ...
        cars.forEach(function(car){car.position.z+=car.userData.speed*dt;if(Math.abs(car.position.x-chickenX)<1.22&&car.position.z>CHICKEN_Z-2.4&&car.position.z<CHICKEN_Z+2.4)car.position.z=ROAD_FAR-8-Math.random()*16;if(car.position.z>12)car.position.z=ROAD_FAR-8-Math.random()*16;car.children.forEach(function(child){if(child.userData&&child.userData.wheel)child.rotation.y+=Math.abs(car.userData.speed)*dt*1.5})});
        if(impactStart){var p=Math.min(1,(time-impactStart)/900),contactAt=.76;if(p<contactAt){var approach=p/contactAt;impactCar.position.z=-38+(CHICKEN_Z+38)*approach}else{var after=(p-contactAt)/(1-contactAt);impactCar.position.z=CHICKEN_Z+after*7;if(!impactContact){impactContact=true;if(impactResolve){var resolve=impactResolve;impactResolve=null;resolve()}}var impact=Math.min(1,after*1.45);chicken.rotation.z=-impact*1.25;chicken.rotation.x=impact*.44;chicken.position.x=chickenX+impact*1.5;chicken.position.y=.02+Math.sin(Math.min(1,impact)*Math.PI)*1.05}if(p>=1){impactStart=0;impactCar.visible=false}}
        else{chicken.rotation.z*=.88;chicken.rotation.x*=.88}
        var follow=1-Math.pow(.003,dt);camera.position.x+=(chickenX-camera.position.x)*follow;camera.lookAt(chickenX,1.0,CHICKEN_Z-4.6);streetGlow.position.x=chickenX;renderer.render(scene,camera)}
