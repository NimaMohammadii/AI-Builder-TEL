...
(function(){
  ...
  function randomInRange(id,range,date){date=date||new Date();var width=range.max-range.min+1;if(width<=1)return range.min;var bucket=Math.floor(date.getTime()/90000);var h=hash(id);var wave=Math.floor((Math.sin((bucket+(profiles[id]&&profiles[id].phase||h))*1.618)+1)*width*.22);var drift=(bucket*37+h*13)%width;return range.min+((drift+wave)%width)}
  function adminCount(id,date){var range=adminRange(id,date);return range?randomInRange(id,range,date):null}
  function seed(id){var admin=adminCount(id);return admin===null?ranged(id):admin}
  ...
  function count(id){if(adminRange(id)!==null){counts[id]=seed(id);return counts[id]}var fromCard=cardValue(id);if(fromCard!==null&&inCurrentRange(id,fromCard)){counts[id]=fromCard;return fromCard}if(!counts[id]||!inCurrentRange(id,counts[id]))counts[id]=seed(id);return counts[id]}
  ...
  function loadAdminSchedule(){return fetch('/app/api/online-user-counts',{credentials:'same-origin',cache:'no-store'}).then(function(r){return r.ok?r.json():null}).then(function(j){if(j&&j.schedule){adminSchedule=j.schedule;counts={};refreshCounts();syncCards()}}).catch(function(){})}
  function syncCards(){if(document.hidden)return;if(!isPlayZoneActive()&&!activeGame()){renderBadge();return}Object.keys(games).forEach(function(id){var current=cardValue(id);if(adminRange(id)!==null)setCount(id,seed(id));else if(current!==null&&inCurrentRange(id,current))counts[id]=current;else setCount(id,count(id))});renderBadge()}
