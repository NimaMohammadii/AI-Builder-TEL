...
(function(){
  ...
  function adminCount(id,date){if(!adminSchedule||!adminSchedule[id])return null;date=date||new Date();var n=Math.floor(Number(adminSchedule[id][date.getHours()]));return isFinite(n)?Math.max(0,n):null}
  ...
  function inCurrentRange(id,value){var n=Math.floor(Number(value));if(!isFinite(n))return false;var admin=adminCount(id);if(admin!==null)return n===admin;var r=rangeForGameHour(id,(new Date()).getHours());return n>=r.min&&n<=r.max}
  ...
  function loadAdminSchedule(){return fetch('/app/api/online-user-counts',{credentials:'same-origin',cache:'no-store'}).then(function(r){return r.ok?r.json():null}).then(function(j){if(j&&j.schedule){adminSchedule=j.schedule;refreshCounts()}}).catch(function(){})}
