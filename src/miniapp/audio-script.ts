...
const VEXA_VOICE_MESSAGE_SCRIPT = `
(function(){
  ...
  function playNext(){
    ...
    busy=true;
    current=message;
    if(message.requiresTap){showToast(message);return}
    playMessage(message,false);
  }

  function playMessage(message,fromTap){
    ensureUi();
    current=message;
    audio.src=message.url;
    var p=audio.play();
    if(p&&typeof p.catch==='function'){
      p.then(function(){if(fromTap)hideToast()}).catch(function(){showToast(message)});
    }else if(fromTap){hideToast()}
  }

  async function trigger(eventId){
    try{
      var message=await fetchMessage(eventId);
      if(message)enqueue(message);
    }catch(e){}
  }

  function activeViewId(){var n=document.querySelector('.view.active');return n&&n.id||''}
  function triggerForView(id){
    if(id==='playzone')trigger('playzone_intro');
    if(id==='predictzone')trigger('predict_intro');
  }
  ...
  function watchClicks(){
    document.addEventListener('click',function(ev){
      ...
      if(view==='playzone'||view==='predictzone')setTimeout(function(){triggerForView(view)},320);
      if(action==='open-daily-guide')setTimeout(function(){trigger('daily_rewards_intro')},320);
    },true);
  }

  function watchFinanceSuccess(){
    var depositStatus=q('depositStatus');
    var withdrawSuccess=q('withdrawSuccess');
    if(depositStatus){
      new MutationObserver(function(){
        if(depositStatus.classList.contains('success')||/payment received/i.test(depositStatus.textContent||''))trigger('first_deposit');
      }).observe(depositStatus,{childList:true,characterData:true,subtree:true,attributes:true,attributeFilter:['class']});
    }
    if(withdrawSuccess){
      new MutationObserver(function(){
        if(withdrawSuccess.classList.contains('show')||withdrawSuccess.getAttribute('aria-hidden')==='false')trigger('first_withdraw');
      }).observe(withdrawSuccess,{attributes:true,attributeFilter:['class','aria-hidden']});
    }
  }
...
export const MINIAPP_AUDIO_SCRIPT = LEVEL_SYNC_SCRIPT + VEXA_VOICE_MESSAGE_SCRIPT;
