...
(function(){
  ...
  function resolvePegCollision(ball,peg,left,right,prevX,prevY){
    ...
    ball.vx*=.996;
    ball.vy*=.996;
    if(Math.abs(ball.vx)<.018&&Math.abs(ball.vy)<.18){
      ball.vx+=(nx>=0?.08:-.08);
      ball.vy=.32;
    }
  ...
  function tick(time){
    ...
    for(var b=balls.length-1;b>=0;b--){var ball=balls[b];ball.age=(ball.age||0)+dt;if(ball.sinking){ball.sink+=dt;if(Number.isFinite(ball.settleX))ball.x+=clamp(ball.settleX-ball.x,-1.2,1.2)*dt;ball.y+=.46*dt;ball.r*=.978;if(ball.sink>42||ball.r<1.2)balls.splice(b,1);continue}
      ...
      if(ball.y>12&&ball.y<binTop-8&&speed<.055){
        ball.stuckFrames=(ball.stuckFrames||0)+1;
      }else{
        ball.stuckFrames=0;
      }
      if(ball.stuckFrames>8){
        ball.vx+=(Math.random()<.5?-.18:.18);
        ball.vy=Math.max(ball.vy,.55);
        ball.y+=1.2;
        ball.stuckFrames=0;
      }
