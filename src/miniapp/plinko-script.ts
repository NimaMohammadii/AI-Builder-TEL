...
(function(){
  ...
  function targetOutletX(index){
    if(!state||!state.pegs||state.pegs.length<3)return 160;
    var slots=houseCount(),idx=clamp(Math.floor(Number(index)||0),0,slots-1);
    var outlet=idx<(slots/3)?0:idx>=(slots*2/3)?2:1;
    return state.pegs[outlet].x;
  }
  function controlledPegNudge(ball,peg,left,right){
    if(!isControlled()||!Number.isFinite(ball.targetIndex)||!state||!state.bins||!state.bins.length)return;
    ball.targetIndex=resolveLandingIndex(ball.targetIndex);
    var target=targetCenterX(ball.targetIndex,state.bins,left,right);
    var rowsLeft=Math.max(1,rows-(Math.floor((peg.key?Number(String(peg.key).split(':')[0]):0)||0)+1));
    var desired=clamp((target-ball.x)/(rowsLeft*34),-.34,.34);
    ball.vx=clamp(ball.vx+desired,-1.15,1.15);
  }
  ...
  function tick(time){
    ...
    for(var b=balls.length-1;b>=0;b--){var ball=balls[b];ball.age=(ball.age||0)+dt;if(ball.sinking){ball.sink+=dt;if(Number.isFinite(ball.settleX))ball.x+=clamp(ball.settleX-ball.x,-1.2,1.2)*dt;ball.y+=.46*dt;ball.r*=.978;if(ball.sink>42||ball.r<1.2)balls.splice(b,1);continue}
      ...
      if(!Number.isFinite(ball.targetIndex))ball.vx+=(Math.random()-.5)*.014*dt;
      ...
      if(ball.y+ball.r>binTop+5){var physicalIdx=binIndexFromX(ball.x,bins,left,right);var bin=bins[physicalIdx];for(var s=1;s<bins.length;s++){var wall=left+s*(right-left)/bins.length;if(Math.abs(ball.x-wall)<ball.r&&ball.y>binTop-6&&ball.y<binBottom){if(ball.x<wall){ball.x=wall-ball.r;ball.vx=-Math.abs(ball.vx)*.46}else{ball.x=wall+ball.r;ball.vx=Math.abs(ball.vx)*.46}ball.vy*=.84}}if(ball.y+ball.r>bin.y+bin.h*.62){ball.settleX=clamp(ball.x,bin.x+ball.r,bin.x+bin.w-ball.r);ball.vx*=.2;ball.vy*=.2;settle(ball,bin);ball.sinking=true;ball.sink=0}}
      if(ball.y>316){if(!ball.paid){var fallbackPhysicalIdx=binIndexFromX(ball.x,bins,left,right);settle(ball,bins[fallbackPhysicalIdx])}balls.splice(b,1)}}
