...
function adminPanelWithFixes(): string {
  return mobileAdminPanelHtml()
    .replace('<button data-credit-adjust="-100">-100</button><input data-credit-value type="number" min="0" value="\'+credit+\'"/><button data-credit-adjust="100">+100</button></div><button class="save-credit" data-credit-save>Set credit</button>', '<button data-credit-minus>-</button><input data-credit-value type="number" min="0" placeholder="Amount" value=""/><button data-credit-plus>+</button></div>')
    .replace("panel.querySelectorAll('[data-credit-adjust]').forEach(btn=>btn.onclick=()=>adjustCredit(userId,Number(btn.getAttribute('data-credit-adjust')||0)));panel.querySelector('[data-credit-save]').onclick=()=>setCredit(userId,Number(panel.querySelector('[data-credit-value]').value||0));", "panel.querySelector('[data-credit-minus]').onclick=()=>adjustCredit(userId,-Math.max(0,Math.floor(Number(panel.querySelector('[data-credit-value]').value||0))));panel.querySelector('[data-credit-plus]').onclick=()=>adjustCredit(userId,Math.max(0,Math.floor(Number(panel.querySelector('[data-credit-value]').value||0))));")
...
export function adminPanelHtml(): string {
  return adminPanelWithFixes().replace('</body></html>', ADMIN_LAYOUT_CSS + ADMIN_IMAGE_PANEL_SCRIPT + ADMIN_UPLOAD_CACHE_SCRIPT + ADMIN_PLINKO_CONTROL_SCRIPT + '</body></html>');
}
