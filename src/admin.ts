...
const ADMIN_GROUPED_USERS_SCRIPT = `<script>(function(){
  ...
  function title(src){return src==='ai_bot'?'AI Bot Users':src==='game_bot'?'Game Bot Users':src==='user_bot'?'User Bot Users':'Other Users'}
  function money(u){return u.tonBalance||fmt(u.tonBalanceNano||u.credit||0)}
  function row(u){return '<article class="user" id="user-'+esc(u.id)+'"><div class="user-top"><div class="user-name"><b>'+esc(u.username||'—')+'</b><span class="badge '+(u.isActive?'on':'off')+'">'+esc(u.status||'Inactive')+'</span></div><strong>'+esc(money(u))+'</strong></div><div class="user-info"><span>ID '+esc(u.id)+'</span><span>'+esc(u.firstName||'—')+'</span><span>'+esc(u.sourceLabel||title(u.source))+'</span><span>'+ago(u.lastSeenAt)+'</span></div><button class="reset-user-btn" data-reset-user="'+esc(u.id)+'" type="button">Reset user</button></article>'}
  async function renderGrouped(){var list=document.getElementById('usersList'),status=document.getElementById('usersStatus'),search=document.getElementById('userSearch');if(!list)return;try{if(status)status.textContent='Loading grouped users...';var r=await fetch('/admin/api/users',{credentials:'same-origin'}),j=await r.json().catch(function(){return{error:'Invalid response'}});if(!r.ok)throw new Error(j.error||'Could not load users');var q=String(search&&search.value||'').trim().toLowerCase();var users=(j.users||[]).filter(function(u){return !q||String(u.id).toLowerCase().includes(q)||String(u.username).toLowerCase().includes(q)||String(u.firstName).toLowerCase().includes(q)||String(u.sourceLabel||u.source||'').toLowerCase().includes(q)});var order=['ai_bot','game_bot','user_bot','unknown'];list.innerHTML=order.map(function(src){var g=users.filter(function(u){return (u.source||'unknown')===src});return g.length?'<h3 class="admin-user-group-title">'+title(src)+' · '+g.length+'</h3>'+g.map(row).join(''):''}).join('')||'<div class="empty">No users found.</div>';if(document.getElementById('statTotal'))document.getElementById('statTotal').textContent=String(j.stats&&j.stats.total||users.length);if(document.getElementById('statOnline'))document.getElementById('statOnline').textContent=String(j.stats&&j.stats.online||0);if(document.getElementById('statInactive'))document.getElementById('statInactive').textContent=String(j.stats&&j.stats.inactive||0);if(document.getElementById('statCredit'))document.getElementById('statCredit').textContent=fmt(j.stats&&j.stats.totalTonBalanceNano||0);if(status)status.textContent='Grouped users updated '+new Date().toLocaleTimeString();document.querySelectorAll('[data-reset-user]').forEach(function(btn){btn.onclick=function(){resetUser(btn.getAttribute('data-reset-user'))}})}catch(e){if(status)status.textContent=e.message||'Could not load users'}}
  async function resetUser(userId){if(!userId)return;if(!confirm('Reset this user? '+userId))return;var status=document.getElementById('usersStatus');if(status)status.textContent='Resetting user...';try{var r=await fetch('/admin/api/users/'+encodeURIComponent(userId),{method:'DELETE',credentials:'same-origin'}),j=await r.json().catch(function(){return{error:'Invalid response'}});if(!r.ok)throw new Error(j.error||'Could not reset user');await renderGrouped()}catch(e){if(status)status.textContent=e.message||'Could not reset user'}}
  window.VexaRenderGroupedUsers=renderGrouped;
  setTimeout(renderGrouped,250);setTimeout(renderGrouped,900);var s=document.getElementById('userSearch');if(s)s.addEventListener('input',function(){setTimeout(renderGrouped,30)});var r=document.getElementById('refreshUsers');if(r)r.addEventListener('click',function(){setTimeout(renderGrouped,30)},true);
})();</script>`;
