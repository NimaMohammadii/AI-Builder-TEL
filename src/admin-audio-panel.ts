...
(function(){
  const AUDIO_TYPES=['audio/mpeg','audio/mp3','audio/wav','audio/x-wav','audio/ogg','audio/webm','audio/mp4','audio/aac'];
  ...
  function render(data){
    ...
    if(meta)meta.textContent=has?('Type: '+(data.type||'audio')+' • Version: '+(data.version||'1')):'MP3, WAV, OGG, WEBM, MP4 or AAC up to 10MB.';
  ...
  document.getElementById('uploadMiniAudio').onclick=async()=>{const input=document.getElementById('miniAudioFile');const file=input&&input.files&&input.files[0];if(!file){setStatus('Choose an audio file first.');return}if(AUDIO_TYPES.indexOf(file.type)<0){setStatus('Only MP3, WAV, OGG, WEBM, MP4 or AAC audio files are allowed.');return}setStatus('Uploading audio...');const form=new FormData();form.append('audio',file);try{const r=await fetch('/admin/api/miniapp-audio',{method:'POST',body:form,credentials:'same-origin'});const j=await r.json().catch(()=>({error:'Upload failed'}));if(!r.ok){setStatus(j.error||'Upload failed.');return}render(j);setStatus('Audio uploaded. Use Play to activate it in the mini app.');}catch(e){setStatus('Upload request failed.');}};
