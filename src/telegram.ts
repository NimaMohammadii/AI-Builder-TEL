...
type BuilderState = { step: 'idle' } | { step: 'waiting_prompt' };
...
async function handleFlowMessage(env: Env, token: string, bot: BotRecord, flow: BotFlow, message: TelegramMessage): Promise<void> {
  ...
  const nextId = node.next ?? (node.end ? flow.start : state.nodeId);
  state.nodeId = nextId;
  await saveFlowState(env, bot.id, userId, state);
  await sendFlowNode(env, token, bot, flow, chatId, userId, state);
}
...
async function handleFlowCallback(env: Env, token: string, bot: BotRecord, flow: BotFlow, callback: TelegramCallbackQuery): Promise<void> {
  ...
  const nextId = data.slice('flow:'.length);
  const state = await getFlowState(env, bot.id, userId, flow);
...
async function sendFlowNode(env: Env, token: string, bot: BotRecord, flow: BotFlow, chatId: number, userId: string, state: UserFlowState): Promise<void> {
  ...
  const text = renderTemplate(node.message, state.data);
  ...
  if (node.end) {
    const resetState = { nodeId: flow.start, data: {} };
    await saveFlowState(env, bot.id, userId, resetState);
  }
}
...
async function getFlowState(env: Env, botId: string, userId: string, flow: BotFlow): Promise<UserFlowState> {
  ...
  if (!flow.nodes[parsed.nodeId]) return { nodeId: flow.start, data: parsed.data ?? {} };
  return { nodeId: parsed.nodeId, data: parsed.data ?? {} };
}
...
async function clearFlowState(env: Env, botId: string, userId: string): Promise<void> {
  await env.BOT_CACHE.delete(`flow-state:${botId}:${userId}`).catch(() => undefined);
}
...
async function saveBotUser(env: Env, botId: string, message: TelegramMessage): Promise<void> {
  ...
  if (!user) return;
  try {
    await env.DB.prepare(`INSERT INTO bot_users (bot_id, telegram_user_id, first_name, username, updated_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP) ON CONFLICT(bot_id, telegram_user_id) DO UPDATE SET first_name = excluded.first_name, username = excluded.username, updated_at = CURRENT_TIMESTAMP`)
      .bind(botId, String(user.id), user.first_name ?? null, user.username ?? null).run();
  } catch (error) { console.warn('saveBotUser failed', error); }
}

async function runtimeAiReply(env: Env, systemPrompt: string, text: string): Promise<string> {
  const { aiReply } = await import('./ai');
  return aiReply(env, systemPrompt, text);
}
...
function mainBuilderBlueprint(): BotBlueprint {
  return { version: 1, botType: 'custom', language: 'en', tone: 'premium', startScreen: 'home', screens: [{ id: 'home', title: 'AI Builder TEL', message: 'Open the Mini App to build Telegram bots without code.', buttons: [] }], aiSupport: { enabled: false, systemPrompt: '', handoffMessage: 'Message saved.' }, safety: { blockedTopics: [], requireHumanFor: [] } };
}
